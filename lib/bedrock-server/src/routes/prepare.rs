...
use bedrock_core::{PrepareRequest, PrepareResult};
use telemetry::tracing::info;
use crate::{
    app_state::AppState, 
    artifacts::resolve_test,
    artifacts::prepare_databases,
    artifacts::clear_nats, 
};
...
pub async fn prepare_route(
    ...
    let response = PrepareResult {
        success: true,
        message: format!("Preparation complete for {}, please conduct the test and hit /tests to execute test", &recording_id),
        recording_id,
