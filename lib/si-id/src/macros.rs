...
macro_rules! id_with_postgres_types {
    (
        $(#[$($attrs:tt)*])*
        $name:ident
    ) => {
        ...
        do_not_use_directly_impl_postgres_types!(
            $(#[$($attrs)*])*
            $name
        );
    };
...
macro_rules! id_with_postgres_and_sea_orm_types {
    (
        $(#[$($attrs:tt)*])*
        $name:ident
    ) => {
        ...
        #[cfg(feature = "sea-orm-types")]
        do_not_use_directly_impl_sea_orm_types!(
            $(#[$($attrs)*])*
            $name
        );
    };
...
/// do you ask?
macro_rules! id_with_none_and_postgres_types {
    (
        $(#[$($attrs:tt)*])*
        $name:ident
    ) => {
...
#[allow(missing_docs)]
macro_rules! do_not_use_directly_impl_base {
    (
        $(#[$($attrss:tt)*])*
        $name:ident
    ) => {
...
// NOTE(nick): same note as the above.
#[allow(missing_docs)]
macro_rules! do_not_use_directly_impl_none {
    (
        $(#[$($attrs:tt)*])*
        $name:ident
    ) => {
...
#[allow(missing_docs)]
#[cfg(feature = "postgres-types")]
macro_rules! do_not_use_directly_impl_postgres_types {
    (
        $(#[$($attrs:tt)*])*
        $name:ident
    ) => {
...
#[allow(missing_docs)]
#[cfg(feature = "sea-orm-types")]
macro_rules! do_not_use_directly_impl_sea_orm_types {
    (
        $(#[$($attrs:tt)*])*
        $name:ident
    ) => {
            ...
            fn try_from(s: String) -> Result<Self, Self::Error> {
                Ok($name(
                    ::ulid::Ulid::from_string(&s)
                        .map_err(|err| sea_orm::DbErr::Type(err.to_string()))?,
                ))
            }
        }
