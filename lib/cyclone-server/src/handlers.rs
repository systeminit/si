...
pub async fn ws_execute_remote_shell(
    ...
    async fn handle_socket(
        ...
        // Follow cyclone protocol: send Start message first
        let start_message = Message::<RemoteShellResultSuccess>::Start;
        let start_text = match serde_json::to_string(&start_message) {
            Ok(text) => text,
            Err(err) => {
                request_span.record_err(&err);
                warn!("failed to serialize start message; error={}", err);
                return;
            }
        };
        
        if let Err(err) = socket.send(ws::Message::Text(start_text)).await {
            request_span.record_err(&err);
            warn!("failed to send start message; error={}", err);
            return;
        }

        // Wait for client to send the request
        let request = match socket.recv().await {
            Some(Ok(ws::Message::Text(text))) => {
                match serde_json::from_str::<CycloneRequest<RemoteShellRequest>>(&text) {
        ...
        // Handle the remote shell session with real process spawning
        if let Err(err) = handle_remote_shell_session(socket, request).await {
            request_span.record_err(&err);
            warn!("remote shell session failed; error={}", err);
        } else {
            request_span.record_ok();
        }
    }
