...
use si_data_nats::{ConnectionMetadata, Message};
...
impl Bifrost {
    pub async fn start(
        ...
        let workspace_messages = tokio_stream::wrappers::BroadcastStream::new(nats_workspace_messages);
        let deployment_messages = tokio_stream::wrappers::BroadcastStream::new(nats_deployment_messages);
                ...
                maybe_nats_message_result = self.nats_messages.next() => {
                    ...
                    match maybe_nats_message_result {
                        ...
                        Some(Ok(nats_message)) => {
                            ...
                            match Self::send_ws_client_message(ws_client, dbg!(ws_message)).await {
                                // Web socket closed, tear down
                                Some(Ok(_)) => {
                                    debug!(
                                        "before sending response, web socket client has closed; {}",
                        ...
                        // We have a `RecvError`
                        None => {
                            dbg!("nats listener has closed; bifrost is probably shutting down");
                        }
                    }
                }
                else => break,
