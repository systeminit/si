<template>
  ...
  >
    <div
      :class="clsx('flex-1 truncate py-2xs pr-xs', variant === 'navbar' && 'px-2xs')"
    >
      <template v-if="modelValue && alwaysShowPlaceholder"><span class="text-neutral-400">{{ placeholder }}</span> {{ modelValueLabel }}</template>
