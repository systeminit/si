...
pub const DBNAME: &str = "split_snapshot_supergraphs";
pub const CACHE_NAME: &str = "split_snapshot_supergraphs";
pub const PARTITION_KEY: &str = "split_snapshot_supergraphs";

#[derive(Debug, Clone)]
pub struct SplitSnapshotSuperGraphDb<V>
where
...
impl<V> SplitSnapshotSuperGraphDb<V>
where
    V: Serialize + DeserializeOwned + Clone + Send + Sync + 'static,
{
    ...
    #[instrument(level = "debug", skip_all,fields(
        si.layer_cache.split_snapshot_supergraph.write_serialize = Empty,
    ))]
    pub fn write(
        &self,
        value: Arc<V>,
        ...
        span.record(
            "si.layer_cache.split_snapshot_supergraph.write_serialize",
        ...
        let event = LayeredEvent::new(
            ...
            Arc::new("split_snapshot_supergraph".to_string()),
            web_events,
            tenancy,
            actor,
        );
    ...
    #[instrument(
        name = "split_snapshot_supergraph.read",
        level = "debug",
        skip_all,
        fields(
            si.split_snapshot_supergraph.address = %key,
        )
    )]
    ...
    #[instrument(
        name = "split_snapshot_supergraph.read_wait_for_memory",
        level = "debug",
        skip_all,
        fields(
    ...
    #[instrument(
        name = "split_snapshot_supergraph.evict",
        level = "debug",
        skip_all,
        fields(
            si.split_snapshot_supergraph.address = %key,
    ...
    #[instrument(
        name = "split_snapshot_supergraph.read_bytes_from_durable_storage",
        level = "debug",
        skip_all,
        fields(
            si.split_snapshot_supergraph.address = %key,
    ...
    pub async fn write_bytes_to_durable_storage(
        ...
        self.cache
            .pg()
            .insert(&key, "split_snapshot_supergraph", bytes)
            .await?;
