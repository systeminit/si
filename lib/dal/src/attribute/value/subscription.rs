...
impl ValueSubscription {
    ...
    /// Find or create the attribute value a subscription points to
    pub async fn vivify(&self, ctx: &DalContext) -> AttributeValueResult<AttributeValueId> {
        self.path.vivify(ctx, self.attribute_value_id).await
    }
