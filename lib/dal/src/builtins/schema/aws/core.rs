...
use crate::{
    attribute::context::AttributeContextBuilder, schema::SchemaUiMenu, ActionPrototype,
    ActionPrototypeContext, AttributeContext, AttributePrototypeArgument, AttributeReadContext,
    AttributeValue, BuiltinsResult, CodeGenerationPrototype, CodeLanguage, ConfirmationPrototype,
...
impl BuiltinSchemaDriver {
    ...
    async fn migrate_ami(&self, ctx: &DalContext) -> BuiltinsResult<()> {
        let (schema, schema_variant, root_prop) =
        ...
        let code_generation_func_id = self
            .get_func_id("si:generateAwsJSON")
            .ok_or(BuiltinsError::FuncNotFoundInCache("si:generateAwsJSON"))?;
    ...
    async fn migrate_region(&self, ctx: &DalContext) -> BuiltinsResult<()> {
        ...
        let region_implicit_internal_provider =
            InternalProvider::get_for_prop(ctx, *region_prop.id())
                .await?
                .ok_or_else(|| {
