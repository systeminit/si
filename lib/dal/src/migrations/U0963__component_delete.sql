...
BEGIN
    ...
    FOR target_pk, peer_component_id, internal_provider_id, external_provider_id IN
        ...
        FROM edges_v1(this_read_tenancy, this_visibility_with_deleted) e
    ...
    FOR target_pk, table_name IN
        ...
        FROM (SELECT pk, 'attribute_prototypes' as table_name, visibility_deleted_at, visibility_change_set_pk
              FROM attribute_prototypes_v1(this_read_tenancy, this_visibility_with_deleted)
              WHERE attribute_context_component_id = this_component_id
              UNION
              SELECT pk, 'attribute_values' as table_name, visibility_deleted_at, visibility_change_set_pk
              FROM attribute_values_v1(this_read_tenancy, this_visibility_with_deleted)
              WHERE attribute_context_component_id = this_component_id
              UNION
              SELECT pk, 'attribute_prototype_arguments' as table_name, visibility_deleted_at, visibility_change_set_pk
              FROM attribute_prototype_arguments_v1(this_read_tenancy, this_visibility_with_deleted)
    ...
    FOR target_pk, table_name IN
        ...
        FROM node_belongs_to_component_v1(this_read_tenancy, this_visibility_with_deleted) nbtc
        WHERE nbtc.belongs_to_id = this_component_id
        ...
        FROM node_belongs_to_component_v1(this_read_tenancy, this_visibility_with_deleted) nbtc
                 INNER JOIN nodes_v1(this_read_tenancy, this_visibility_with_deleted) n ON n.id = nbtc.object_id
        ...
        FROM node_belongs_to_component_v1(this_read_tenancy, this_visibility_with_deleted) nbtc
                 INNER JOIN node_position_belongs_to_node_v1(this_read_tenancy, this_visibility_with_deleted) npbtn
                            ON nbtc.object_id = npbtn.belongs_to_id
        ...
        FROM node_belongs_to_component_v1(this_read_tenancy, this_visibility_with_deleted) nbtc
                 INNER JOIN node_position_belongs_to_node_v1(this_read_tenancy, this_visibility_with_deleted) npbtn
                            ON nbtc.object_id = npbtn.belongs_to_id
                                AND npbtn.visibility_deleted_at IS NOT NULL
                                AND npbtn.visibility_change_set_pk = (this_visibility ->> 'visibility_change_set_pk')::ident
                 INNER JOIN node_positions_v1(this_read_tenancy, this_visibility_with_deleted) np
    ...
    SELECT pk
    INTO target_pk
    FROM components_v1(this_read_tenancy, this_visibility_with_deleted)
