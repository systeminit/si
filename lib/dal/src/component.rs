...
impl Component {
    ...
    /// Return the ids of all the components managed by this component
    pub async fn get_managed_by_id(ctx: &DalContext, component_id: ComponentId) -> ComponentResult<Vec<ComponentId>> {
        let mut result = vec![];

        let snapshot = ctx.workspace_snapshot()?;

        for target_idx in snapshot
            .outgoing_targets_for_edge_weight_kind(component_id, EdgeWeightKindDiscriminants::Manages)
            .await?
        {
