...
use crate::{
    workspace_snapshot::graph::detector::Change, DalContext, HistoryActor, TransactionsError,
...
impl ChangeSetApproval {
    ...
    pub async fn new(ctx: &DalContext, status: ChangeSetApprovalStatus) -> Result<Self> {
        ...
        let (checksum, _) = Self::calculate_checksum(ctx).await?;
        let row = ctx
            .txns()
            .await?
            .pg()
    ...
    pub async fn calculate_checksum(ctx: &DalContext) -> Result<(ChangesChecksum, Vec<Change>)> {
        let mut changes = ctx
            .workspace_snapshot()?
            .detect_changes_from_head(ctx)
            .await?;

        // We must sort the changes by ID before continuing!
