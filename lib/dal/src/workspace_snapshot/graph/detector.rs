...
use crate::{
    workspace_snapshot::{
        content_address::ContentAddressDiscriminants, node_weight::NodeWeight, NodeInformation,
    },
...
#[derive(Debug)]
pub enum ChangeKind {
    Content(ContentAddressDiscriminants),
    Node(NodeWeightDiscriminants),
}
            ...
            match self.updated_graph.get_node_weight(updated_graph_index) {
                Ok(updated_node_weight) => {
                    ...
                    changes.push(Change {
                        ...
                        kind: match updated_node_weight.content_address_discriminants() {
                            Some(content_address_discrim) => ChangeKind::Content(content_address_discrim),
                            None => ChangeKind::Node(updated_node_weight.into())
                        },
