...
impl SubscriptionGraph {
    ...
    fn component_id_for_av(
        ...
        let node_weight = snapshot.get_node_weight(av_idx).ok()?;
        ...
        let component_idx = snapshot
            .source_opt(current_idx, EdgeWeightKindDiscriminants::Root)
            .ok()
            .flatten()
            .or_else(|| {
                // Also check SocketValue edge for socket AVs, although these are deprecated
                snapshot
                    .source_opt(current_idx, EdgeWeightKindDiscriminants::SocketValue)
                    .ok()
                    .flatten()
            })?;

        let component_weight = snapshot.get_node_weight(component_idx).ok()?;
        if !matches!(component_weight, NodeWeight::Component(_)) {
            return None;
        }

        Some(component_weight.id().into())
    }
