...
async fn create_and_list_for_schema_variant(ctx: &DalContext<'_, '_>) {
    ...
    // FIXME(nick): this setup is straight up wrong. For now, we just want to make
    ...
    let _internal_provider =
        InternalProvider::new_implicit(ctx, *value_prop.id(), *schema.id(), *schema_variant.id())
            .await
...
async fn intra_component_identity_update(ctx: &DalContext<'_, '_>) {
    ...
    let (_, updated_destination_attribute_value_id, _) = AttributeValue::update_for_context(
        ctx,
    ...
    let (_, twice_updated_source_attribute_value_id, _) = AttributeValue::update_for_context(
        ctx,
