...
async fn attribute_value_names(
    ...
    for child_av_id in AttributeValue::get_child_av_ids_in_order(ctx, av_id).await? {
        result.push(
            AttributeValue::prop_for_id(ctx, child_av_id)
                .await?
                .expect("prop")
                .name,
        );
...
const EXPECT_ABCDEF: [&'static str; 6] = ["a", "b", "c", "d", "e", "f"];
const EXPECT_FEDCBA: [&'static str; 6] = ["f", "e", "d", "c", "b", "a"];
...
async fn property_order_remains_after_update(ctx: &mut DalContext) -> dal_test::Result<()> {
    ...
    AttributeValue::update(
        &ctx,
        abcdef.attribute_value(ctx).await.id(),
        Some(json!({})),
    )
    .await?;
    AttributeValue::update(
        &ctx,
        fedcba.attribute_value(ctx).await.id(),
        Some(json!({})),
    )
    .await?;
    assert_eq!(attribute_value_names(ctx, abcdef).await?, EXPECT_ABCDEF);
...
async fn child_property_value_remains_after_update_and_paste(
    ...
    // assert_eq!(
    //     EXPECT_ABCDEF,
    //     attribute_value_names(ctx, abcdef).await?
    // );
    // assert_eq!(
    //     EXPECT_FEDCBA,
    //     attribute_value_names(ctx, fedcba).await?
    // );

    // Ensure they have the right values after updating them
    for name in ["a", "b", "c", "d", "e", "f"] {
        let abcdef_id = AttributeValue::update(
            &ctx,
            component
