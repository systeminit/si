...
use dal::{Component, DalContext, Schema, SchemaVariant, SchemaVariantId};
use dal_test::test;
...
async fn create_and_determine_lineage(ctx: &DalContext) {
    ...
    // Get the default schema variant id. Ensure the id is valid.
    let schema_variant_id = schema
        .default_schema_variant_id()
        .expect("no default variant found");
    assert_ne!(
        SchemaVariantId::NONE, // expected
        schema_variant_id      // actual
    );

    // Create a component and set geometry.
    let name = "fsu not top four";
    let mut component = Component::new(&ctx, name, schema_variant_id, None)
        .await
