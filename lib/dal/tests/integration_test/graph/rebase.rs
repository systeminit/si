...
use dal::DalContext;
use dal::{
    change_set_pointer::ChangeSetPointer, workspace_snapshot::content_address::ContentAddress,
};
use dal::{FuncBackendKind, WorkspaceSnapshot};
use dal_test::test;

#[test]
async fn simulate_rebase(ctx: &DalContext) {
    let to_rebase_change_set = ChangeSetPointer::new_local().expect("Unable to create ChangeSet");
