...
use deadpool_cyclone::{
    ...
    ConfirmationRequest, CycloneClient, FunctionResult, Manager, Pool, ProgressMessage,
};
...
async fn main() -> Result<(), Box<(dyn std::error::Error + 'static)>> {
    ...
    let request: ConfirmationRequest = serde_json::from_reader(io::stdin())?;
...
async fn execute(
    ...
    info!(
        ...
        "Executing confirmation check"
    );
    let mut progress = instance
        .execute_confirmation(request)
        .await?
        .start()
        .await?;
