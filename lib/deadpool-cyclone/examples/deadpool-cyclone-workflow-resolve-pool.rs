...
use deadpool_cyclone::{
    ...
    CycloneClient, FunctionResult, Manager, Pool, ProgressMessage, WorkflowResolveRequest,
};
...
async fn main() -> Result<(), Box<(dyn std::error::Error + 'static)>> {
    ...
    let request: WorkflowResolveRequest = serde_json::from_reader(io::stdin())?;
...
async fn execute(
    ...
    match result {
        // TODO(fnichol): add field output when fields are added
        FunctionResult::Success(success) => info!(execution_id = &success.execution_id.as_str(),),
        FunctionResult::Failure(failure) => error!(execution_id = &failure.execution_id.as_str(),),
    }
