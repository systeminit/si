...
use crate::{app_state::AppState, routes::AppError};
...
pub async fn upsert_builtin_route(
    ...
    if !is_systeminit_auth_token(state.auth_api_url(), &auth_token, state.token_emails())
        .await
    ...
    let multiparts = extract_multiparts(&mut multipart)
        .await
        .map_err(anyhow::Error::from)?;
    if let Some(schema_id) = multiparts.schema_id.clone() {
        let existing_schema_id = SchemaId::from_str(&schema_id).map_err(anyhow::Error::from)?;
        reject_other_modules_of_a_schema_id("Clover".to_string(), existing_schema_id, &txn)
            .await
            .map_err(anyhow::Error::from)?;
    }

    // Upload the new module
    let new_module = upsert_module(multiparts, &txn, user_claim, s3_bucket)
        .await
        .map_err(anyhow::Error::from)?;

    // Promote the new module to be a builtin
    promote_module(new_module.id, &txn, "Clover".to_string())
        .await
        .map_err(anyhow::Error::from)?;
