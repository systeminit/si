...
#[utoipa::path(
    delete,
    path = "/v1/w/{workspace_id}/change-sets/{change_set_id}/schemas/{schema_id}/variant/{schema_variant_id}/funcs/action/{func_id}",
    params(
    ...
    tag = "schemas",
    responses(
        (status = 200, description = "Action function successfully deteched from the variant", body = DetachFuncBindingV1Response),
...
pub async fn detach_action_func_binding(
    ...
    for binding in bindings {
        if let FuncBinding::Action(action) = binding {
            ...
            tracker.track(
                ctx,
                "api_delete_action_func_binding",
                serde_json::json!({
                    "func_id": func.id,
