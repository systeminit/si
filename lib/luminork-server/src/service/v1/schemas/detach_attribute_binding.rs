...
#[utoipa::path(
    delete,
    path = "/v1/w/{workspace_id}/change-sets/{change_set_id}/schemas/{schema_id}/variant/{schema_variant_id}/funcs/attribute/{func_id}",
    params(
    ...
    tag = "schemas",
    responses(
        (status = 200, description = "Attribute function successfully detached from the variant", body = DetachFuncBindingV1Response),
...
pub async fn detach_attribute_func_binding(
    ...
    for binding in bindings {
        if let FuncBinding::Attribute(attribute) = binding {
            AttributeBinding::reset_attribute_binding(
                ctx,
                attribute.attribute_prototype_id.into_raw_id().into(),
            )
            .await?;

            tracker.track(
                ctx,
                "api_delete_attribute_func_binding",
                serde_json::json!({
                    "func_id": func.id,
                    "schema_variant_id": schema_variant_id,
                    "attribute_prototype_id": attribute.attribute_prototype_id,
                }),
            );
