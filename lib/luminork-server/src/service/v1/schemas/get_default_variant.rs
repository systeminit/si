        ...
        .await
    {
        ...
        Ok(None) => {
            ...
            if Schema::get_by_id_opt(ctx, schema_id).await?.is_some() {
                ...
                return Ok(SchemaVariantResponseV1::Building(BuildingResponseV1::new_and_increment_counter_for_default_variant_workspace_graph()
                ));
            }
        }
        Err(e) => {
        ...
        None => {
            ...
            match CachedModule::find_latest_for_schema_id(ctx, schema_id).await {
                Ok(Some(_)) => {
                    ...
                    return Ok(SchemaVariantResponseV1::Building(BuildingResponseV1::new_and_increment_counter_for_default_variant_cached_modules()));
                }
                Ok(None) => {
