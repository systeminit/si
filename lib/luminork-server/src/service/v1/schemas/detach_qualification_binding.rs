...
pub async fn detach_qualification_func_binding(
    ...
    for binding in bindings {
        if let FuncBinding::Qualification(qual) = binding {
            if qual.leaf_kind == LeafKind::Qualification {
                match qual.leaf_binding_prototype {
                ...
                tracker.track(
                    ...
                    serde_json::json!({
                        ...
                        "leaf_binding_prototype": qual.leaf_binding_prototype,
                    }),
                );
    ...
    // Delete schema-level overlays for this func that are qualification type
    for leaf_prototype in leaf_prototypes {
        let prototype_func_id = LeafPrototype::func_id(ctx, leaf_prototype.id()).await?;
        if prototype_func_id == func_id && leaf_prototype.kind() == LeafKind::Qualification {
