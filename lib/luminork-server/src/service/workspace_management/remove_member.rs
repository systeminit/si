...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Member removed successfully", body = Vec<Member>),
        (status = 400, description = "Bad Request - Invalid email format"),
        (status = 401, description = "Unauthorized - Invalid or missing token"),
        (status = 403, description = "Forbidden - User must be workspace owner or approver to remove members"),
...
pub async fn remove_member(
    ...
    payload: Result<Json<RemoveMemberRequest>, axum::extract::rejection::JsonRejection>,
