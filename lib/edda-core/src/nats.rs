...
pub mod subject {
    ...
    // - `edda.updates.deployment` (update for an entire deployment)
    // - `edda.updates.workspace.$workspace_id` (update for a workspace)
    // - `edda.updates.change_set.$workspace_id.$change_set_id` (update for a change set)
    ...
    #[inline]
    pub fn all_change_set_updates_for_all_workspaces(prefix: Option<&str>) -> Subject {
        nats_std::subject::prefixed(
            prefix,
            format!("{UPDATES_SUBJECT_PREFIX}.{}.*.*", Scope::ChangeSet.as_ref()),
        )
    }

    #[inline]
    pub fn all_change_set_updates_for_workspace(
        prefix: Option<&str>,
        workspace_id: &str,
    ) -> Subject {
        nats_std::subject::prefixed(
