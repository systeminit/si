...
pub type NewChangeSetRequestVCurrent = NewChangeSetRequestV1;

#[derive(Clone, Eq, Serialize, PartialEq, VariantNames)]
#[serde(rename_all = "camelCase")]
#[strum(serialize_all = "camelCase")]
pub enum NewChangeSetRequest {
    V1(NewChangeSetRequestV1),
}

impl ApiWrapper for NewChangeSetRequest {
    type VersionsTarget = NewChangeSetRequestVersions;
    type Current = NewChangeSetRequestVCurrent;

    const MESSAGE_TYPE: &'static str = "NewChangeSetRequest";

    fn id(&self) -> RequestId {
        match self {
            Self::V1(NewChangeSetRequestVCurrent { id, .. }) => *id,
        }
    }
...
impl fmt::Debug for NewChangeSetRequest {
...
impl Deref for NewChangeSetRequest {
    type Target = NewChangeSetRequestVCurrent;
...
impl DerefMut for NewChangeSetRequest {
...
pub enum NewChangeSetRequestVersions {
    V1(NewChangeSetRequestV1),
}

impl ApiVersionsWrapper for NewChangeSetRequestVersions {
    type Target = NewChangeSetRequest;

    fn id(&self) -> RequestId {
        match self {
            Self::V1(NewChangeSetRequestV1 { id, .. }) => *id,
        }
    }
...
mod test {
    ...
    use super::NewChangeSetRequestVersionsDiscriminants;
    ...
    pub fn assert_serialize(
        name: &str,
        version: NewChangeSetRequestVersionsDiscriminants,
        serialize: impl Serialize,
    ) {
        insta::with_settings!({
            snapshot_path => format!("new_change_set_request/snapshots-{}", version.as_ref()),
        ...
        T: fmt::Debug + DeserializeOwned + PartialEq,
    {
        let actual: T = read_from_snapshot(
            snapshot_name,
            &format!("new_change_set_request/snapshots-{}", version.as_ref()),
        )
