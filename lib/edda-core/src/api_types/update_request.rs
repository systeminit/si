...
pub enum UpdateRequestAllVersions {
    #[acceptable(current)]
    V1(UpdateRequestV1),
}

impl IntoContainer for UpdateRequestAllVersions {
    type Container = UpdateRequest;
...
mod test {
    ...
    pub fn assert_serialize(name: &str, version: u64, serialize: impl Serialize) {
        insta::with_settings!({
            snapshot_path => format!("update_request/snapshots-v{version}"),
        ...
        T: fmt::Debug + DeserializeOwned + PartialEq,
    {
        let actual: T = read_from_snapshot(
            snapshot_name,
            &format!("update_request/snapshots-v{version}"),
        )
