...
pub struct DefaultFrameSocketRequest {
...
pub async fn default_frame_socket(
    HandlerContext(builder): HandlerContext,
    AccessBuilder(request_ctx): AccessBuilder,
    OriginalUri(_original_uri): OriginalUri,
    Host(_host_name): Host,
    PosthogClient(_posthog_client): PosthogClient,
    Json(DefaultFrameSocketRequest {
        component_id,
        socket_id,
        visibility,
    }): Json<DefaultFrameSocketRequest>,
    ...
    let default_connections =
        DefaultConnection::get_default_connections_for_frame(&ctx, component_id).await?;
    ...
    let maybe: Vec<DefaultConnection> = default_connections
        ...
        .filter_map(|conn| match conn {
            DefaultConnection::Frame(component_socket) => {
            ...
            DefaultConnection::Workspace(_) | DefaultConnection::View(_) => None,
        })
        .collect();
    ...
    .await
    {
        ...
        if maybe.is_empty() {
            DefaultConnection::set_default_connection_for_frame(&ctx, attribute_value_id).await?;
        } else {
            DefaultConnection::remove_for_frame(&ctx, attribute_value_id).await?;
        }
    };
