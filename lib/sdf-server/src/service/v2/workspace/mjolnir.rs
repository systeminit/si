...
use crate::{
    ...
    service::v2::workspace::WorkspaceAPIResult,
};
...
pub async fn get_front_end_object(
    ...
    let obj = frigg
        ...
        .await?;
    match obj {
        Some(o) => return Ok(Json(o)),
        None => {
            return Err(super::WorkspaceAPIError::LatestItemNotFound(
                workspace_pk,
                request.kind,
                request.id,
            ));
...
pub async fn get_multiple_front_end_objects(
    ...
    let Some(mv_index_data) = maybe_mv_index else {
        return Err(super::WorkspaceAPIError::IndexNotFound);
    };
    ...
    for object_request in request.requests {
        ...
        join_set.spawn(async move {
            ...
            Ok::<(Result<Option<FrontendObject>, _>, FrontEndObjectRequest), super::WorkspaceAPIError>((
                frigg_clone
                    ...
                    .await.map_err(Into::<super::WorkspaceAPIError>::into),
                object_request,
            ))
        });
    }
