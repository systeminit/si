...
pub async fn assemble_in_list(
    ...
    let name = Component::name_by_id(ctx, component_id)
    ...
    let input_count = ctx
        ...
        .await?;
    let has_diff = Component::has_diff_from_head(ctx, component_id)
        .instrument(info_span!("Component::has_diff_from_head"))
        .await?;
    ...
    // FIXME: delete this
    let _diff_count = Component::get_diff_count(ctx, component_id)
