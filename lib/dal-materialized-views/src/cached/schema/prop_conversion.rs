...
fn generate_uninstalled_prop_ulid(variant_id: SchemaVariantId, prop_path: &str) -> Ulid {
    ...
    let base_ulid = Ulid::from_string(UNINSTALLED_PROP_ID_BASE)
        .expect("Base ULID should be valid");
    let timestamp = base_ulid.timestamp_ms();

    // Create new ULID with same timestamp but path-based randomness
    Ulid::from_parts(timestamp, path_hash as u128)
}
...
mod tests {
    ...
    fn test_base_ulid_is_valid() {
        ...
        assert!(result.is_ok(), "Base ULID should be valid: {}", UNINSTALLED_PROP_ID_BASE);
    }
