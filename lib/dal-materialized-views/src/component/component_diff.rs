    ...
    while let Some((old_av_id, new_av_id)) = work_queue.pop() {
        match (old_av_id, new_av_id) {
            ...
            (None, Some(new_av_id)) => {
                ...
                for new_av_id in
                    AttributeValue::get_child_av_ids_in_order(new_ctx, new_av_id).await?
                {
            ...
            (Some(old_av_id), None) => {
                ...
                for old_av_id in
                    AttributeValue::get_child_av_ids_in_order(old_ctx, old_av_id).await?
                {
...
async fn child_av_pairs(
    ...
    let mut old_children = old_children.into_iter().peekable();
