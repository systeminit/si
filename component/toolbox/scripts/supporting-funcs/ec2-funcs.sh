...
list_instances() {
  ...
  if [[ "${filter,,}" == "all" || -z "${filter}" ]]; then
    ...
    aws ec2 describe-instances --query 'Reservations[*].Instances[?State.Name==`running`].[Tags[?Key==`Name`].Value | [0],InstanceId,InstanceType,PrivateIpAddress]' --output text | grep -E 'sdf|veritech|pinga|rebaser|forklift|edda'
