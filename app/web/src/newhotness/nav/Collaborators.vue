...
const sortedUsers = computed(() => {
  ...
  return usersCopy.sort((a, b) => {
    if (ctx.changeSetId.value) {
      if (a.changeSet !== ctx.changeSetId.value && b.changeSet === ctx.changeSetId.value) {
        return 2;
      }
      if (a.changeSet === ctx.changeSetId.value && b.changeSet !== ctx.changeSetId.value) {
        return -2;
...
const userTooltips = computed(() => {
  ...
  displayUsers.value.forEach((user) => {
    const userChangeSet = openChangeSets.value.find((changeSet) => changeSet.id === user.changeSet);
