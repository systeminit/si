<template>
  <FuncRunListLayout
    ...
    @update:selectedFuncKind="(val) => (selectedFuncKind = val as FuncKind)"
...
// Filter state
const selectedFuncKind = ref<FuncKind | "">("");
...
const funcRuns = computed<FuncRun[]>(() => {
  ...
  if (selectedFuncKind.value) {
    filtered = filtered.filter((funcRun) => funcRun.functionKind === selectedFuncKind.value);
  }
