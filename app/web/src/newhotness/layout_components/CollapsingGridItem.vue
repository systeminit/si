<template>
  <div class="overflow-hidden min-h-0 flex flex-col">
    <h3
      :class="
        clsx(
          ...
          themeClasses('bg-neutral-300 border-neutral-400', 'bg-neutral-800 border-neutral-600'),
          !disableCollapse && ['cursor-pointer', themeClasses('hover:bg-neutral-400', 'hover:bg-neutral-700')],
        )
      "
      @click="toggleOpen"
    >
      <CollapseExpandChevron v-if="!disableCollapse" :open="openState.open.value" class="mr-xs" />
      <slot name="header" />
      <div class="ml-auto" />
    ...
    <div
      :class="
        clsx(
          disableScroll ? 'overflow-hidden flex flex-col min-h-[calc(100%-40px)]' : 'scrollable flex-1 min-h-0',
          funcRunScreen && [
            'border-x border-b',
            themeClasses('bg-neutral-300 border-neutral-400', 'bg-neutral-800 border-neutral-600'),
          ],
