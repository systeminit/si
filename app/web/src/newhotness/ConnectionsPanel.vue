  ...
  <div
    ...
    :class="
      clsx(
        ...
        themeClasses('border-neutral-400 bg-white', 'border-neutral-600 bg-neutral-900'),
      )
    "
  >
    <template v-if="incoming.length">
      <ConnectionLayout label="Incoming" :connections="incoming" :highlightedPath="highlightedPath" />
    </template>
    <template v-if="outgoing.length">
      <ConnectionLayout label="Outgoing" :connections="outgoing" />
    </template>
  </div>
  <EmptyState v-else-if="!inMap" icon="output-connection" text="No connections yet" />
</template>
...
import { IncomingConnections, ComponentInList, BifrostComponent } from "@/workers/types/entity_kind_types";
...
const connections = computed(() => connectionsGetter(props.component.id, props.connections).value);
