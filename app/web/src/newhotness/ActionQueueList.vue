    ...
    <template v-for="section in actionDisplayLists" :key="section.title">
      ...
      >
        <div
          :class="
            clsx(
              'flex flex-row items-center gap-xs w-full',
              themeClasses(
                'text-neutral-600 [&_*]:border-neutral-600',
                'text-neutral-400 [&_*]:border-neutral-400',
              ),
            )
          "
        >
...
const actionDisplayLists = computed(() => [
  ...
  {
    title: "Queued",
    // TODO(Wendy) - need to filter for only top level actions
    actions: props.actionViewList.filter(
      (action) => action.state === ActionState.Queued,
    ) as ActionProposedView[],
  },
  {
    title: "On Hold",
    actions: props.actionViewList.filter(
      (action) => action.state === ActionState.OnHold,
    ) as ActionProposedView[],
  },
]);

const retryAll = () => {
  // TODO(Wendy) - retry all failed actions here!
};
