...
const getReferences = async (
  ...
  if (kind === EntityKind.SchemaVariantCategories) {
    ...
    const incomingResults: Record<Id, BifrostComponentConnections> | -1 =
      await getMany(
        workspaceId,
        changeSetId,
        EntityKind.IncomingConnections,
    ...
    const maybeIncomingConnections: BifrostComponentConnections[] = [];
    for (const connRef of rawList.componentConnections) {
      const result = incomingResults[connRef.id];
      if (result) {
        maybeIncomingConnections.push(result as BifrostComponentConnections);
      } else {
        hasReferenceError = true;
        mjolnir(
          workspaceId,
...
const getMany = async (
  ...
  if ([EntityKind.IncomingConnections].includes(kind)) {
    ...
    const componentIds: Id[] = Object.values(eddaIncConns)
      .flatMap((c) => c.connections)
      .flatMap((r) => [r.toComponentId.id, r.fromComponentId.id]);
    ...
    type MaybeBifrostConnection = Omit<
      BifrostConnection,
    ...
    type MaybeBifrostComponentConnections = Omit<
      BifrostComponentConnections,
      "component" | "incoming"
    > & {
