  ...
  >
    <template v-if="actionProposed">
      ...
      <Icon
        ...
        :class="
          clsx(
            actionProposed.holdStatusInfluencedBy.length > 0
              ? [
                  'opacity-30',
    ...
    >
      ...
      <ol v-if="actionProposed.myDependentActions.length > 0">
        <li
          v-for="a in actionProposed.myDependentActions"
          :key="a.id"
      ...
      <ol v-if="actionProposed.dependentOnActions.length > 0">
        <li
          v-for="a in actionProposed.dependentOnActions"
          :key="a.id"
          class="flex flex-row items-center px-2xs gap-xs"
        >
          <Icon
...
const actionProposed = computed(() => {
  if ("state" in props.action) {
    return props.action as ActionProposedViewWithHydratedChildren;
  } else {
    return undefined;
  }
});
