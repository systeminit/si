...
import { ComponentType, SchemaVariant, UninstalledVariant } from "@/api/sdf/dal/schema";
...
export type CategoryVariant = CategoryInstalledVariant | CategoryUninstalledVariant;
...
const isAttributeSourceSetSubscription = (s: AttributeSource): s is AttributeSourceSetSubscription => {
...
type AttributeSourceUnset = { $source: null } | { $source: { value?: undefined } };

const isAttributeSourceUnset = (s: AttributeSource): s is AttributeSourceUnset => {
  const unsetPayload = s as AttributeSourceUnset;
  return unsetPayload.$source === null || unsetPayload.$source.value === undefined;
};
...
function edgeFromRawEdge({ isInferred, isManagement }: { isInferred?: boolean; isManagement?: boolean }) {
...
export const loadCollapsedData = (prefix: string, key: DiagramElementUniqueKey) => {
...
export function getPossibleAndExistingPeerSockets(
  ...
  const nonDeletedEdges = allEdges.filter((e) => e.def.changeStatus !== "deleted");

  const edgeCountForInputKey = {} as Record<string, number>;
  nonDeletedEdges.forEach((e) => {
    edgeCountForInputKey[e.toSocketKey] = (edgeCountForInputKey[e.toSocketKey] ?? 0) + 1;
  });
  ...
  const existingEdges = nonDeletedEdges
    ...
    .filter(
      ({ thisComponentId, thisSocketId }) => thisComponentId === targetComponentId && thisSocketId === targetSocket.id,
    );
        ...
        c.def.sockets
          ?.filter((peerSocket) => {
            ...
            if (peerSocket.direction === "input") {
              ...
              const edgeCount = edgeCountForInputKey[componentAndSocketKey] ?? 0;

              if (peerSocket.maxConnections && edgeCount >= peerSocket.maxConnections) {
                return false;
              }
            }
            ...
            const [outputCAs, inputCAs] =
              targetSocket.direction === "output"
                ? [targetSocket.connectionAnnotations, peerSocket.connectionAnnotations]
                : [peerSocket.connectionAnnotations, targetSocket.connectionAnnotations];
  ...
  const [existingPeers, possiblePeers] = socketsWithParent.reduce(
    ([existing, possible], socket) => {
      const existingEdge = existingPeersIdsAndEdges[`${socket.componentId}-${socket.id}`];
...
export const processRawComponent = (component: RawComponent, allComponents: Record<ComponentId, RawComponent>) => {
...
export const useComponentsStore = (forceChangeSetId?: ChangeSetId) => {
  ...
  return addStoreHooks(
    workspaceId,
    changeSetId,
    defineStore(`ws${workspaceId || "NONE"}/cs${changeSetId || "NONE"}/components`, {
      state: () => ({
        ...
        allComponentsById: {} as Record<ComponentId, DiagramNodeData | DiagramGroupData>,
      ...
      getters: {
        diagramSubscriptionEdgesById(): Record<EdgeId, DiagramEdgeData> {
          ...
          const rawEdges = _.compact(edges.map((edge) => processRawEdge(edge, this.allComponentsById))).map((edge) => [
            edge.def.id,
            edge,
          ]);
        ...
        componentsByParentId(): Record<ComponentId, (DiagramGroupData | DiagramNodeData)[]> {
          return _.groupBy(this.allComponentsById, (c) => c.def.parentId ?? "root");
        },
        categories(): Categories {
          const assetStore = useAssetStore();
          const installedGroups = _.groupBy(assetStore.variantList, "category");
          const uninstalledGroups = _.groupBy(assetStore.uninstalledVariantList, "category");

          const mergedKeys = _.uniq([...Object.keys(installedGroups), ...Object.keys(uninstalledGroups)]);
          ...
          return mergedKeys
            .map((category) => {
              ...
              const schemaVariants: CategoryVariant[] = [...uninstalledVariants, ...installedVariants];
              schemaVariants.sort((a, b) =>
                (a.variant.displayName || a.variant.schemaName)?.localeCompare(
        ...
        possibleAndExistingPeerSocketsFn: (state) => {
          ...
          const allEdges = _.values(state.diagramEdgesById).filter((e) => e instanceof DiagramSocketEdgeData); // TODO upgrade typescript in web app and remove this cast
          const peerCache = {};
          return (targetSocket: DiagramSocketDef, targetComponentId: ComponentId) =>
            getPossibleAndExistingPeerSockets(targetSocket, targetComponentId, allComponents, allEdges, peerCache);
        },
      },
      actions: {
        ...
        processAndStoreRawComponent(
          ...
          if (processChildren) {
            const children = Object.values(this.allComponentsById).filter((c) => c.def.parentId === component.id);
            ...
            onSuccess: (payload) => {
              if (payload.potentialIncoming.length === 0) {
                ...
                this.autoconnectData = {
                  ...
                  potentialConnections: payload.potentialIncoming.map((pc) => {
                    const socket = thisComponent.def.sockets?.find((s) => s.id === pc.socketId);
                    return {
                      socketId: pc.socketId,
                      socketName: socket?.label || "",
                      socketArity: socket?.maxConnections === 1 ? "one" : "many",
                      ...
                      processingConnections: pc.matches.map(
                        (m: {
                          ...
                          const otherComponent = this.allComponentsById[m.componentId];
                          const otherSocket = otherComponent?.def.sockets?.find((s) => s.id === m.socketId);
                          return {
                            socketId: m.socketId,
                            socketName: otherSocket?.label,
                            socketArity: otherSocket?.maxConnections === 1 ? "one" : "many",
                            componentId: m.componentId,
                            componentName: otherComponent?.def.displayName || "",
                            schemaVariantName: otherComponent?.def.schemaName || "",
                            value: m.value,
                            state: "PENDING",
        ...
        SET_COMPONENTS_FROM_VIEW(
          ...
          if (options.representsAllComponents) {
            ...
            const idsToDelete = existingIds.filter((id) => !allIds.includes(id));
          ...
          const edges =
            response.edges && response.edges.length > 0
              ? response.edges.map(edgeFromRawEdge({ isInferred: false, isManagement: false }))
              : [];
          const inferred =
            response.inferredEdges && response.inferredEdges.length > 0
              ? response.inferredEdges.map(edgeFromRawEdge({ isInferred: true, isManagement: false }))
              : [];

          const management =
            response.managementEdges?.length > 0
              ? response.managementEdges.map(edgeFromRawEdge({ isInferred: false, isManagement: true }))
              : [];

          const subscription =
            featureFlagsStore.SIMPLE_SOCKET_UI && response.attributeSubscriptionEdges?.length > 0
              ? response.attributeSubscriptionEdges.map(edgeFromRawEdge({ isInferred: false, isManagement: false }))
              : [];

          const edgesToSet = [...edges, ...inferred, ...management, ...subscription];
          if (options.representsAllComponents) {
        ...
        async OVERRIDE_WITH_CONNECTION(
          ...
          return new ApiRequest({
            ...
            optimistic: () => {
              ...
              const edgesBeingReplaced = Object.values(this.rawEdgesById).filter(
                (e) =>
                  e.isInferred && isSocketEdge(e) && e.toSocketId === to.socketId && e.toComponentId === to.componentId,
              );
        ...
        async UPDATE_COMPONENT_ATTRIBUTES(componentId: ComponentId, payload: UpdateComponentAttributesArgs) {
          return new ApiRequest({
            method: "put",
            ...
            optimistic: () => {
              for (const toPath in payload) {
                ...
                if (isAttributeSourceSetSubscription(update)) {
                  ...
                  const edgesBeingReplaced = Object.values(this.rawEdgesById).filter(
                    (e) => isSubscriptionEdge(e) && e.toAttributePath === toPath && e.toComponentId === componentId,
                  );
                ...
                if (isAttributeSourceUnset(update)) {
                  const edgesBeingDeleted = Object.values(this.rawEdgesById).filter(
                    (e) => isSubscriptionEdge(e) && e.toAttributePath === toPath && e.toComponentId === componentId,
                  );
        ...
        async UPGRADE_COMPONENT(componentId: ComponentId, componentName: string) {
        ...
        async DELETE_EDGE(
          ...
          const url = edge?.isManagement ? "component/unmanage" : "diagram/delete_connection";
        ...
        async DELETE_COMPONENTS(componentIds: ComponentId[], forceErase = false) {
          if (changeSetsStore.creatingChangeSet) {
            throw new Error("race, wait until the change set is created");
          }
          if (changeSetId === changeSetsStore.headChangeSetId) {
        ...
        async CREATE_TEMPLATE_FUNC_FROM_COMPONENTS(templateData: {
          ...
          const { color, assetName, funcName, componentIds, viewId, category } = templateData;
        ...
        setComponentDisplayName(component: DiagramGroupData | DiagramNodeData, name: string) {
        ...
        realtimeStore.subscribe(`${this.$id}-changeset`, `changeset/${changeSetId}`, [
          ...
          },
          {
            ...
            callback: (data, metadata) => {
              ...
              const oldParent = this.rawComponentsById[componentId]?.parentId;
          ...
          },
          {
            ...
            callback: (data) => {
              if (data.changeSetId !== changeSetId) return;
              const edges =
                data.edges && data.edges.length > 0 ? data.edges.map(edgeFromRawEdge({ isInferred: true })) : [];
