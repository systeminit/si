...
import { EntityKind, GLOBAL_ENTITIES, GLOBAL_IDENTIFIER, GlobalEntity } from "@/workers/types/entity_kind_types";
  ...
  return <K = Gettable>(
    ...
    >(() => {
      if (GLOBAL_ENTITIES.includes(kind as GlobalEntity)) {
        return [GLOBAL_IDENTIFIER, toValue(kind), toValue(id) ?? GLOBAL_IDENTIFIER];
      } else
        return extension
          ? [
          ...
          : [ctx.workspacePk.value, ctx.changeSetId.value, toValue(kind), toValue(id ?? ctx.workspacePk)];
    });
};
