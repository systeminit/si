<template>
  ...
  >
    ...
    <div
      ...
      class="fixed right-0 top-[60px] bg-gray-900 shadow-2xl z-50 flex flex-col border-l border-gray-700"
      :style="{ width: sidebarWidth + 'px', height: `calc(100vh - 60px)` }"
    >
      <!-- Resize handle -->
      <div
        ref="resizeHandle"
      ...
      <!-- Status Bar -->
      <div
        class="px-3 py-1 bg-gray-800 text-xs text-gray-400 border-t border-gray-700"
      >
        <div class="flex justify-between">
          <span>{{ connectionStatus }}</span>
          <span v-if="sessionDetails">{{
            sessionDetails.sessionId.substring(0, 8)
          }}</span>
        </div>
      </div>
    </div>
    ...
    >
      ...
      >
        <div class="flex items-center space-x-2">
          ...
          <span class="text-sm font-medium text-gray-300 ml-2"
            >Terminal - {{ sessionDetails?.sessionId || "Connecting..." }}</span
          >
          <div
            class="w-2 h-2 rounded-full"
            :class="
      ...
      >
        <div class="flex justify-between">
          <span
            ...
            {{ sessionDetails?.sessionId || "N/A" }}</span
          >
  ...
  terminal = new Terminal({
    ...
    cols:
      panelState.value === "popped-out"
        ? 160
        : Math.floor(sidebarWidth.value / 9), // Approximate chars per pixel
