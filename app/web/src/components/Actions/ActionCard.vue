...
const actionFailed = computed(() => {
  if ("state" in props.action) return props.action.state === ActionState.Failed;
  else return false;
});
...
const actionQueued = computed(() => {
  if ("state" in props.action) return props.action.state === ActionState.Queued;
  else return false;
});
...
const hydrateActions = (actionList: ActionId[] | undefined) => {
  ...
  if (actionList) {
    for (const id of actionList) {
      ...
      const a = _a as ActionViewWithComponent;
      if (a) {
        if (a.componentId) {
...
const dependentOn = computed(() => hydrateActions(props.action.dependentOn));
const myDependencies = computed(() =>
  hydrateActions(props.action.myDependencies),
);
const holdStatusInfluencedBy = computed(() =>
  hydrateActions(props.action.holdStatusInfluencedBy),
);
