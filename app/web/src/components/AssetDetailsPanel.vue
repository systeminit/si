<template>
  <div class="grow relative">
    <ScrollArea v-if="editingAsset && schemaVariantId" ref="scrollAreaRef">
      <template #top>
        <div class="flex flex-row items-center justify-around gap-xs p-xs border-b dark:border-neutral-600">
          <VButton
            :disabled="saveAssetReqStatus.isPending || editingAsset.isLocked || assetStore.codeSaveIsDebouncing"
        ...
        <AssetNameModal ref="cloneAssetModalRef" buttonLabel="Clone Asset" title="Asset Name" @submit="cloneAsset" />
        <ErrorMessage
        ...
        <AssetFuncAttachModal ref="attachModalRef" :schemaVariantId="props.schemaVariantId" />
      </template>
      <Stack class="p-xs" spacing="none">
        ...
        <VormInput
          ...
          @focus="focus"
        />
        <VormInput id="componentType" disabled compact label="Component Type" type="text" :modelValue="'Component'" />
        <VormInput
          id="description"
          ...
          @focus="focus"
        />
        <VormInput :disabled="editingAsset.isLocked" compact label="Color" type="container">
          <ColorPicker
            id="color"
        ...
        <Stack class="p-xs" spacing="none">
          ...
          <div class="text-xs pb-xs">
            <div class="flex items-center">
              ...
              <IconButton
                class="ml-xs"
                :icon="showIntrinsicFuncOptionsText ? 'chevron--down' : 'chevron--right'"
                iconIdleTone="neutral"
                size="sm"
                @click="toggleIntrinsicFuncOptionsText"
              />
            </div>
            <ul v-if="showIntrinsicFuncOptionsText" class="list-disc pl-md py-xs">
              <li><strong>Unset:</strong> the value is unset</li>
              <li><strong>Identity:</strong> get the value from another prop or input socket without modifying it</li>
              <li>
                <strong>Normalize to Array:</strong> get the value from another prop or input socket, but modify it if
                the input is empty (becomes "[]") or not an array (wrapped with "[]")
              </li>
            </ul>
          </div>
          <span class="uppercase font-bold text-sm">Output Sockets</span>
          <ul v-if="outputSocketIntrinsics.length > 0">
            <li v-for="config in outputSocketIntrinsics" :key="config.attributePrototypeId">
          ...
          <p v-else class="text-xs pb-4 pt-2">No output sockets exist for asset.</p>
        </Stack>
        <Stack class="p-xs" spacing="none">
    ...
    <div v-else class="px-2 py-sm text-center text-neutral-400 dark:text-neutral-300">
      <template v-if="props.schemaVariantId">Asset "{{ props.schemaVariantId }}" does not exist! </template>
      <template v-else>Select an asset to view its details.</template>
    </div>
    <Modal
      ref="executeAssetModalRef"
      :title="editingAsset && editingAsset.schemaVariantId ? 'Asset Updated' : 'New Asset Created'"
      size="sm"
...
import { InputSocketId, SchemaVariant, SchemaVariantId } from "@/api/sdf/dal/schema";
...
const intrinsics = computed(() => {
  ...
  const intrinsics = funcStore.intrinsicBindingsByVariant.get(props.schemaVariantId);
...
const _configurableProps = computed(() => {
  ...
  variant?.funcIds.forEach((funcId) => {
    ...
    if (summary?.kind === FuncKind.Intrinsic) if (INTRINSICS_DISPLAYED.includes(summary.backendKind)) return; // ignore set string, etc
  ...
  >;
  intrinsics.value
    .filter((binding): binding is BindingWithBackendKindAndPropId => !!binding.propId)
    .forEach((binding) => {
      const arg = binding.argumentBindings.filter((a) => !!a.propId).pop();
      const inputSocket = binding.argumentBindings.filter((a) => !!a.inputSocketId).pop();
      if (arg && arg.propId)
  ...
  _props.forEach(({ id, path, name }) => {
    ...
    else if (backendKind === FuncBackendKind.NormalizeToArray) funcId = normalizeToArrayFuncId.value;
...
const _outputSocketIntrinsics = computed(() => {
  ...
  intrinsics.value
    .filter((binding): binding is BindingWithBackendKindAndOutputSocket => !!binding.outputSocketId)
    .forEach((binding) => {
      const arg = binding.argumentBindings.filter((a) => !!a.propId).pop();
      const inputSocket = binding.argumentBindings.filter((a) => !!a.inputSocketId).pop();

      const socketName =
        assetStore.selectedSchemaVariant?.outputSockets.find((s) => s.id === binding.outputSocketId)?.name || "N/A";

      let value;
      if (arg && arg.propId) value = `p_${arg.propId}`;
      if (inputSocket && inputSocket.inputSocketId) value = `s_${inputSocket.inputSocketId}`;

      let funcId = unsetFuncId.value;
      if (binding.backendKind === FuncBackendKind.Identity) funcId = identityFuncId.value;
      else if (binding.backendKind === FuncBackendKind.NormalizeToArray) funcId = normalizeToArrayFuncId.value;
...
const commonBindingConstruction = (data: PropDisplay | IntrinsicDisplay): Attribute | undefined => {
  if (!props.schemaVariantId) return;
  ...
  if (data.funcId === normalizeToArrayFuncId.value) funcArgumentId = normalizeToArrayFuncArgumentId.value;

  const argumentBindings: AttributeArgumentBinding[] = [];

  if (data.funcId === identityFuncId.value || data.funcId === normalizeToArrayFuncId.value) {
    ...
    if (data.value)
      if (data.value.startsWith("s_")) arg.inputSocketId = data.value.replace("s_", "");
      else if (data.value.startsWith("p_")) arg.propId = data.value.replace("p_", "");
  ...
  if (binding) {
    ...
    if (!resp.result.success) {
      if (resp.result.statusCode === 422) {
        toast("Error: chosen prop configuration is invalid. It would cause a cycle");
        configurableProps.value = toRaw(_configurableProps.value);
      }
    }
  }
...
const updateOutputSocketIntrinsics = async (data: IntrinsicDisplay) => {
  ...
  if (binding) {
    ...
    if (!resp.result.success) {
      if (resp.result.statusCode === 422) {
        toast("Error: chosen socket configuration is invalid. It would cause a cycle");
        outputSocketIntrinsics.value = toRaw(_outputSocketIntrinsics.value);
      }
    }
  }
...
watch(
  ...
  () => {
    ...
    if (!data) return;
    if (focusedFormField.value) delete data[focusedFormField.value as keyof SchemaVariant];
...
const updateAssetReqStatus = assetStore.getRequestStatus("REGENERATE_VARIANT", assetStore.selectedVariantId);
const saveAssetReqStatus = assetStore.getRequestStatus("SAVE_SCHEMA_VARIANT", assetStore.selectedVariantId);
...
const cloneAsset = async (name: string) => {
  if (editingAsset.value?.schemaVariantId) {
    const result = await assetStore.CLONE_VARIANT(editingAsset.value.schemaVariantId, name);
    if (result.result.success) {
      cloneAssetModalRef.value?.modal?.close();
    } else if (!result.result.success && result.result.statusCode === 409) {
      cloneAssetModalRef.value?.setError("That name is already in use, please choose another");
    }
