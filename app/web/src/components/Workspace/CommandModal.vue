<template>
  <Modal ref="commandModal" noWrapper size="4wxl" @close="close">
    <section class="bg-white rounded-md text-black p-md">
      <ol id="construction" class="flex flex-row w-full mb-md">
        <template
          v-for="(cmd, index) in commandBuffer"
          :key="`${cmd}-${index}`"
        >
          <li
            class="cmd buffer mr-xs mb-xs text-md rounded-md bg-action-200 px-xs py-[.33rem]"
          ...
          <li
            ...
            :class="
              clsx(
                'buffer mr-xs mb-xs text-md rounded-md bg-action-200 px-xs py-[.33rem]',
      ...
      <ul
        v-if="!maybeDone && dropDownOptions.length > 0"
        id="dropdown"
        ref="dropdown"
        :style="dropdownStyle"
        class="rounded-sm min-h-[15vh] max-h-[30vh] min-w-[10vh] max-w-[15vh] overflow-y-scroll bg-white"
      >
        <li
          ...
          :class="
            clsx(
              selectedOption?.value === option.value ? 'text-action-600' : '',
            )
          "
...
const commandInput = ref<InstanceType<typeof HTMLHeadingElement>>();
...
const filter = (event: KeyboardEvent) => {
  ...
  if (event.code === "ArrowDown") {
    ...
    dropDownFilter.value = commandInput.value?.innerText.trim();
  }
};
...
const input = (event?: Event) => {
  ...
  const str = commandInput.value?.innerText.trim();
  ...
  // reset input
  if (commandInput.value) commandInput.value.innerText = "";
...
#commandInput {
  ...
  padding-left: 1px; // shows cursor
  anchor-name: --commandInput;
}
