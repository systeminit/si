          ...
          >
            <template #label>
              <TruncateWithTooltip class="text-sm">
                {{ schemaDisplayName(schemaVariant, category.schemaVariants) }}
              </TruncateWithTooltip>
              <!-- <div
...
const schemaDisplayName = (
  ...
  const duplicates = schemaVariants.filter(
    (s) => s.schemaName === schemaVariant.schemaName,
  );
  let displayName = schemaVariant.displayName;
  if (!displayName) displayName = schemaVariant.schemaName;
  if (duplicates.length > 1) {
    return `${displayName} (${duplicates.indexOf(schemaVariant)})`;
