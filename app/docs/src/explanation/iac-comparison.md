...
:::tip
Web Frameworks like [React](https://react.dev) or [Vue](https://vuejs.org/) have a similar concept of reactive subscriptions. They facilitate efficient updates, easy tracing, and enable a range of internal debugging and visualization tools.
...
The result is that we do not need a global lock, because state cannot conflict. We utilize this feature to enable our Web Application to work in a real-time way - updates are shared immediately between all users.
...
This is a huge boon, in particular when you are writing more complex automation using our [public API](../reference/public-api.md). We use this feature to do things like trigger increases in tracing for applications during troubleshooting, etc. The sky's the limit.
...
When you change the Image in the TaskDefinition, it will automatically schedule an Update to it. Because that update will change the ARN of the deployed Task, any AWS::ECS::Service that is running and subscribed to it will also automatically update.
:::
