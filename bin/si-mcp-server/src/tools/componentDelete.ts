import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { CallToolResult } from "@modelcontextprotocol/sdk/types.js";
import { z } from "zod";
import { ComponentsApi } from "@systeminit/api-client";
import { apiConfig, WORKSPACE_ID } from "../si_client.ts";
import {
  errorResponse,
  generateDescription,
  successResponse,
} from "./commonBehavior.ts";

const name = "component-delete";
const title = "Delete a component";
const description =
  `<description>Delete a component for a given componentId. Returns the deletion status on successful deletion. On failure, returns error details.</description><usage>Use this tool to delete a component in a change set. The component will be marked for deletion and removed from the workspace.</usage>`;

const DeleteComponentInputSchemaRaw = {
  changeSetId: z.string().describe(
    "The change set to delete the component from; components cannot be deleted from the HEAD change set",
  ),
  componentId: z.string().describe("the component id to delete"),
};

const DeleteComponentOutputSchemaRaw = {
  status: z.enum(["success", "failure"]),
  errorMessage: z.string().optional().describe(
    "If the status is failure, the error message will contain information about what went wrong",
  ),
  data: z.object({
    status: z.string().describe("the deletion status (e.g., 'MarkedForDeletion')"),
  }),
};
const DeleteComponentOutputSchema = z.object(
  DeleteComponentOutputSchemaRaw,
);

type DeleteComponentResult = z.infer<
  typeof DeleteComponentOutputSchema
>["data"];

export function componentDeleteTool(server: McpServer) {
  server.registerTool(
    name,
    {
      title,
      description: generateDescription(
        description,
        "componentDeleteResponse",
        DeleteComponentOutputSchema,
      ),
      inputSchema: DeleteComponentInputSchemaRaw,
      outputSchema: DeleteComponentOutputSchemaRaw,
    },
    async (
      { changeSetId, componentId },
    ): Promise<CallToolResult> => {
      const siApi = new ComponentsApi(apiConfig);
      try {
        const response = await siApi.deleteComponent({
          workspaceId: WORKSPACE_ID,
          changeSetId: changeSetId,
          componentId,
        });
        const result: DeleteComponentResult = {
          status: response.status,
        };

        return successResponse(
          result,
        );
      } catch (error) {
        return errorResponse(error);
      }
    },
  );
}
