...
const UpdateComponentOutputSchema = createResponseSchema({
...
export function componentUpdateTool(server: McpServer) {
  server.registerTool(
    name,
    {
      ...
      outputSchema: UpdateComponentOutputSchema.shape,
    },
    async ({
      changeSetId,
      attributes,
      ...
      try {
        ...
        const deepLink = generateComponentDeepLink(changeSetId, componentId);

        return successResponse(result, undefined, deepLink);
      } catch (error) {
