async function main(component: Input): Promise<Output> {
  ...
  if (!subscriptionId || !tenantId || !clientId || !clientSecret) {
    throw new Error(
      "Azure credentials not found (hint: you may need a secret with AZURE_SUBSCRIPTION_ID, AZURE_TENANT_ID, AZURE_CLIENT_ID, AZURE_CLIENT_SECRET)",
    );
  }
  ...
  if (!resourcePath) {
    return {
      status: "error",
      message: "No resource path found in domain configuration",
    };
  }
  ...
  const url =
    `https://management.azure.com${resourcePath}?api-version=${apiVersion}`;
...
function cleanPayload(domain: any): any {
