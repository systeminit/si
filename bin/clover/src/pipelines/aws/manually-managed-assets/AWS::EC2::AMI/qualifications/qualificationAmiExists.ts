async function main({ domain: { region, Filters, ExecutableUsers, Owners, ImageId, UseMostRecent }}: Input): Promise < Output > {
  ...
  if (queryArgs.length > 0) {
    ...
    if (imageIds.length > 1 && !UseMostRecent) {
      return {
        result: "failure",
        message: "Multiple images match the query! Either narrow the query, or set UseMostRecent=true."
      };
    }
    ...
    if (ImageId != imageIds[0]) {
      return {
        result: "failure",
        message: `ImageId incorrect or out of date (query returned ${imageIds[0]})`
      }
    }
    return {
      result: "success",
