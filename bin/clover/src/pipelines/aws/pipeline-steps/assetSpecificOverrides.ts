...
const SCHEMA_OVERRIDES = new Map<string, OverrideFn>([
  ...
  [
    ...
    (spec: ExpandedPkgSpec) => {
      const targetGroupArnToListAttributeFunc = Deno.readTextFileSync(
        "./src/pipelines/aws/funcs/overrides/Listener DefaultActions/attribute/setTargetGroupArns.ts",
      );
  ...
  [
    ...
    (spec: ExpandedPkgSpec) => {
      ...
      const { func, leafFuncSpec } = attachQualificationFunction(
        "./src/pipelines/aws/funcs/overrides/VPNConnection VpnTunnelOptionsSpecifications/qualifications/presharedKeyValidations.ts",
