...
async function emulate_authoring_inner(sdf: SdfApiClient, changeSetId: string) {
  ...
  schemaVariantId = await updateAssetCode(
    sdf,
    changeSetId,
    schemaVariantId,
    doublerAssetCode,
  ...
  let doublerVariant = await sdf.changeSetMjolnir(
    changeSetId,
    "SchemaVariant",
    schemaVariantId,
  );
  ...
  const outputProp = doublerProps.find(
    (p: any) => p.path === "root/domain/doubled",
  );
  assert(outputProp, "Expected to find doubled prop");
  let inputProp: any = doublerProps.find(
    (p: any) => p.path === "root/domain/input",
  );
  ...
  const doubleFuncId = await createAttributeFunction(
    sdf,
    changeSetId,
    "double",
    doublerVariant.id,
  ...
  await eventualMVAssert(
    ...
    (mv) =>
      Object.values(mv.attributeValues).some(
        (av: any) => av.path === "/domain/doubled" && av.value === "4",
      ),
  ...
  await eventualMVAssert(
    ...
    schemaVariantId,
    (mv) =>
      Object.values(mv.propTree.props).some(
        (p: any) => p.path === "root/domain/tripled",
      ),
    "SchemaVariant should have new tripled prop",
  );

  doublerVariant = await sdf.changeSetMjolnir(
    changeSetId,
    "SchemaVariant",
    schemaVariantId,
  );
  ...
  // Add an attribute function
  const tripleProp = doublerProps.find(
    (p: any) => p.path === "root/domain/tripled",
  );
  ...
  const tripleFuncId = await createAttributeFunction(
    sdf,
    changeSetId,
    "triple",
    schemaVariantId,
  ...
  await eventualMVAssert(
    ...
    (mv) =>
      Object.values(mv.attributeValues).some(
        (av: any) => av.path === "/domain/tripled" && av.value === "6",
      ),
...
async function createAttributeFunction(
  ...
  for (const funcArg of funcArguments) {
    ...
    assert(Array.isArray(createArgResp?.arguments), "Expected arguments list");
    numArgs++;
    assert(
      createArgResp?.arguments.length === numArgs,
      `Expected ${numArgs} but found ${createArgResp?.arguments.length}`,
    );
    const createdArg = args.find((arg) => arg.name === funcArg.name);
    const attributePrototypeArgumentId = createdArg.id;
    const attributePrototypeId =
      createArgResp?.bindings[0].attributePrototypeId;
