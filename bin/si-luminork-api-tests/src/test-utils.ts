...
export async function loadConfig(cliParams: {
  ...
  console.log(`- API URL: ${envApiUrl ? 'Environment' : envFileVars['LUMINORK_API_URL'] ? '.env file' : cliParams.apiUrl ? 'CLI param' : 'Default'}`);
  console.log(`- Auth Token: ${envAuthToken ? 'Environment' : envFileVars['LUMINORK_AUTH_TOKEN'] ? '.env file' : cliParams.authToken ? 'CLI param' : 'Not set'}`);
  console.log(`- Workspace ID: ${envWorkspaceId ? 'Environment' : envFileVars['LUMINORK_WORKSPACE_ID'] ? '.env file' : cliParams.workspaceId ? 'CLI param' : 'Not set'}`);
  console.log(`- Timeout: ${envTimeout ? 'Environment' : envFileVars['LUMINORK_TIMEOUT'] ? '.env file' : cliParams.timeout !== undefined ? 'CLI param' : 'Default'}`);

  // Validate required configuration - fail if missing
  if (!authToken) {
    throw new ConfigError('LUMINORK_AUTH_TOKEN is required but not set in environment, .env file, or CLI parameters');
  }

  if (!workspaceId) {
    throw new ConfigError('LUMINORK_WORKSPACE_ID is required but not set in environment, .env file, or CLI parameters');
  }
...
export async function createTestClient(cliParams: {
  ...
  try {
    ...
    if (error instanceof ConfigError) {
      ...
      console.error('Make sure to set configuration via environment variables, .env file, or CLI parameters');
      throw error;
    }
    throw error;
  }
 ...
 */
export async function cleanupTestResources(api: LuminorkApi, workspaceId: string, changeSetIds: string[]): Promise<void> {
