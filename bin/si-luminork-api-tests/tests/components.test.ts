...
import { createTestClient, generateTestName, cleanupTestResources, ConfigError } from '../src/test-utils.ts';
...
Deno.test("Components API - Create and Update Components", async () => {
  try {
    ...
    try {
      ...
      // Use AWS::EC2::Instance as the schema name as requested
      const schemaName = "AWS::EC2::Instance";
      ...
      const isComponentInList = listComponentsResponse.data.components.includes(componentId);
      assertEquals(isComponentInList, true, `Component ${componentId} not found in the list`);
      ...
      assertEquals(findComponentResponse.status, 200, "Find component response status should be 200");
      assertEquals(findComponentResponse.data.component.name, updatedName, "Find component names should be equal");
      ...
      try {
        ...
        const response = await api.components.getComponent(config.workspaceId, changeSetId, componentId);
        ...
        throw new Error('Component should have been deleted - was still able to retrieve it');
      } catch (error) {
        // Check if this is an ApiError with 404 status (expected case)
        if (error instanceof ApiError && error.status === 404) {
          console.log(`Verified component was deleted successfully - got expected 404 Not Found response`);
        } 
        // The API might return error differently than expected - check error message for 404 references
        else if (error instanceof Error && 
                (error.message.includes("404") || 
                 error.message.includes("Not Found"))) {
          console.log("Successfully deleted component - error message contains 404 Not Found reference");
        }
