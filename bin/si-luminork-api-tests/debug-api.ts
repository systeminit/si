...
async function testSystemStatus() {
  ...
  console.log("\nSystem Status Test Result:", result["What is this?"]);
}
...
async function testListSchemas(changeSetId: string) {
  const result = await apiRequest(`/v1/w/${workspaceId}/change-sets/${changeSetId}/schemas`);
...
async function testGetSchema(changeSetId: string, schemaId: string) {
  const result = await apiRequest(`/v1/w/${workspaceId}/change-sets/${changeSetId}/schemas/${schemaId}`);
...
async function runTests() {
  try {
    ...
    if (changeSetResult && changeSetResult.changeSet && changeSetResult.changeSet.id) {
      ...
      console.log("Failed to create change set or missing change set ID in response");
    }
  } catch (error) {
