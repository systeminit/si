 ...
 * ### Project Root Marker
 *
 * A conduit project root is identified by the presence of a `.conduitroot` file
 * in the directory. This marker file indicates that the directory is the root
 * of a System Initiative conduit project.
 *
 * ## Error Handling
 *
 ...
 * - A directory does not contain a `.conduitroot` marker file
 * - No `.conduitroot` file is found when searching upward through the directory
 *   tree
 */
export class RootPathNotFoundError extends ValidationError {
   ...
   *
   * @param path - The path where the `.conduitroot` marker file was not found
   */
...
export class RootPath {
   ...
   * @throws {ValidationError} If no `.conduitroot` file is found in the current
   ...
   *    provided)
   * 2. Checks if a `.conduitroot` file exists in the current directory
   ...
   * @throws {ValidationError} If no `.conduitroot` file is found in the start
   * directory or any parent directory up to the filesystem root
   */
   ...
   * `.conduitroot` marker file, without searching parent directories. This is
   ...
   * @returns A RootPath instance if the path contains a `.conduitroot` file, or
 ...
 * validate that command-line path arguments point to valid conduit project
 * roots (directories containing a `.conduitroot` marker file).
 *
 * When used in a Cliffy command definition, this type:
 * 1. Accepts a path string from the command line
 * 2. Validates that the path contains a `.conduitroot` marker file
...
export class RootPathType extends Type<RootPath | RootPathNotFoundError> {
   ...
   * arguments. It validates that the provided path contains a `.conduitroot`
   * marker file.
   *
   ...
   * RootPathNotFoundError if the path does not contain a `.conduitroot` file
   */
