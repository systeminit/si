...
export async function exportAssets(
  ...
  try {
    ...
    analytics.trackEvent("export_assets", {
      exportedSchemasCount: exportedSchemas,
      exportedSchemaNames: schemas.map((schema) => schema.name),
      workspaceId,
      changeSetId,
      changeSetUrl,
    });
    ...
    console.log(
      `${exportedSchemas} schemas exported. To see them, go to: ${changeSetUrl}`,
    );
