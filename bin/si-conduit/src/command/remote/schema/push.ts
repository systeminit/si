      ...
      for (const schemaData of schemasToPush) {
        ...
        for (
          ...
          if (existingFuncId) {
            ...
            await createFunc(siSchemasApi, logger, baseVariantPayload, filesystemFuncKind, filesystemFunc);
          }
        }
...
export async function callRemoteSchemaOverlaysPush(
  ...
  for (const schemaName of overlaySchemaNames) {
    ...
    if (localOverlayAuth.length > 0) {
      logger.warn(`${localOverlayAuth.length} auth functions found in overlays, but authentication funcs are not supported. Skipping...`);
    }
    ...
    const localOverlayFuncs = [
      ...
      ...localOverlayCodegen.map(f => ({ kind: "CodeGeneration", funcData: f })),
      ...localOverlayManagement.map(f => ({ kind: "Management", funcData: f })),
      ...localOverlayQualification.map(f => ({ kind: "Qualification", funcData: f })),
    ];

    // Compare and categorize functions
    const funcsToCreate = [];
    const funcsToUpdate = [];
    const funcsToUnbindByName = {} as Record<string, { id: string; name: string; kind: string }>;

    // Track all remote functions for unbinding
    for (const [name, { id, data }] of Object.entries(existingOverlayFuncByName)) {
    ...
    logger.info("");
    logger.info(`  Summary: ${funcsToCreate.length} to create, ${funcsToUpdate.length} to update, ${Object.keys(funcsToUnbindByName).length} to unbind`);

    if (funcsToCreate.length === 0 && funcsToUpdate.length === 0 && Object.keys(funcsToUnbindByName).length === 0) {
  ...
  await wrapInChangeSet(
    ...
    logger,
    workspaceId,
    "Conduit Overlays Push",
    async (changeSetId) => {
      for (let schema of schemasToPush) {
        ...
        for (let { name: funcName, id: funcId, kind } of schema.funcsToUnbind) {
          logger.trace(`${kind} "${funcName}": detaching`);
        ...
        for (let { kind, data: funcData } of schema.funcsToCreate) {
          const funcName = funcData.name;
        ...
        for (let { id: funcId, data: funcData, kind } of schema.funcsToUpdate) {
          const funcName = funcData.name;
