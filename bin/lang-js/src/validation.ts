...
import {failureExecution, FunctionKind, RequestWithCode, ResultFailure, ResultSuccess,} from "./function";
...
export async function executor<Req extends RequestWithCode, Result>(
   ...
   debug({ code });
    ...
    const vm = createNodeVm(
        createSandbox(kind, request.executionId)
    );
...
async function execute(
    ...
    if (typeof result["message"] !== "string" && typeof result["message"] !== "undefined") {
         return {
            protocol: "result",
            status: "failure",
            executionId,
