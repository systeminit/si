...
export default async function create_two_components_connect_and_propagate(sdfApiClient: SdfApiClient) {
  return runWithTemporaryChangeset(sdfApiClient, create_two_components_connect_and_propagate_inner);
}

async function create_two_components_connect_and_propagate_inner(sdf: SdfApiClient, changeSetId: string) {
  // CREATE COMPONENTS
  // get schema variant ids
  const schemaVariants = await sdf.call({
    route: "schema_variants", routeVars: {
  ...
  const ec2InstanceVariant = schemaVariants.find((sv) => sv.schemaName === "EC2 Instance");
  const ec2InstanceVariantId = ec2InstanceVariant?.schemaVariantId;
  assert(ec2InstanceVariantId, "Expected to find EC2 Instance schema and variant");

  const awsRegionVariant = schemaVariants.find((sv) => sv.schemaName === "Region" && sv.category === "AWS");

  const awsRegionVariantId = awsRegionVariant?.schemaVariantId;
  assert(awsRegionVariantId, "Expected to find Generic Frame schema and variant");

  // create region component
  let createRegionPayload = {
  ...
  assert(diagram.components.length === 2, "Expected a single component on the diagram");

  const regionComponentOnDiagram = diagram.components.find((c) => c.id === regionComponentId);
  assert(regionComponentOnDiagram, "Expected to find region on the diagram");
  assert(regionComponentOnDiagram?.schemaVariantId === awsRegionVariantId, "Expected diagram component schema variant id to be correct");

  const instanceComponentOnDiagram = diagram.components.find((c) => c.id === instanceComponentId);
  assert(instanceComponentOnDiagram, "Expected to find region on the diagram");
  assert(instanceComponentOnDiagram?.schemaVariantId === ec2InstanceVariantId, "Expected diagram component schema variant id to be correct");

  // CONNECT COMPONENTS
  const outputSocket = awsRegionVariant.outputSockets.find((s) => s.name === "Region");
  assert(outputSocket?.id, "Expected to find region output socket");
  const inputSocket = ec2InstanceVariant.inputSockets.find((s) => s.name === "Region");
  ...
  assert(diagramWithConnection.edges.length === 1, "Expected a single edge on the diagram");

  const edge = diagramWithConnection.edges[0];
  assert(
    edge.fromComponentId === regionComponentId && edge.toComponentId === instanceComponentId,
    "Expected edge to be between the right components"
  );

  // UPDATE SOURCE VALUE
  // get source prop
  const sourceRegionProp = awsRegionVariant.props.find(p => p.path === "/root/domain/region");
  ...
  const sourcePropValues = await sdf.call({
    route: "get_property_values", routeVars: {
      componentId: regionComponentId,
      changeSetId,
    }
  });
  assert(typeof sourcePropValues?.values === "object", "Expected prop values");
  assert(typeof sourcePropValues?.childValues === "object", "Expected prop childValues:");

  let sourceAttributeValue;
  for (const attributeValue in sourcePropValues?.values) {
    if (sourcePropValues?.values[attributeValue]?.propId === sourceRegionProp.id) {
  ...
  assert(sourceAttributeValueParent, "Expected parent of source attribute value");
  ...
  // get source prop
  const destinationRegionProp = ec2InstanceVariant.props.find(p => p.path === "/root/domain/region");
  ...
  await retryWithBackoff(async () => {
    ...
    assert(typeof destinationPropValues?.values === "object", "Expected prop values");
    ...
    assert(destinationRegionValue.value === regionValue, "Expected propagated value to match source");
  });
}
