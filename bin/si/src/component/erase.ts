/**
 * Component Erase Module - Erase components in a change set
 *
 * This module provides functionality to erase components from the System Initiative
 * API. It supports erasure by component ID or name, and includes a dry-run mode
 * to preview what would be erased. The command gracefully handles components that
 ...
 */
export interface ComponentEraseOptions {
 ...
 */
export async function callComponentErase(
  componentIdOrName: string,
  options: ComponentEraseOptions,
): Promise<void> {
  const ctx = Context.instance();
  const apiConfig = Context.apiConfig();
  const workspaceId = Context.workspaceId();

  ctx.logger.info(`Erasing component: {component}`, {
  ...
  try {
    ...
    if (statusCode === 404) {
      ...
      ctx.logger.warn(
        `Component '{component}' not found - already gone or never existed`,
        { component: componentIdOrName },
      );
  ...
  ctx.logger.info(
    `Successfully erased component: {name} ({id})`,
    {
      name: component.name,
      id: component.id,
    },
