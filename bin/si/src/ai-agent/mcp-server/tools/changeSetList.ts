...
const description = `<description>Lists change sets. Returns the ID, Name and Status of the change set. On failure, returns error details</description><usage>Use this tool when you need to know what change sets are available in the workspace.</usage>`;
const hints =
...
const ListChangeSetsOutputSchema = z.object(ListChangeSetsOutputSchemaRaw);
    ...
    async (): Promise<CallToolResult> => {
      return await withAnalytics(name, async () => {
        ...
        try {
          ...
          return successResponse(response.data.changeSets, hints);
        } catch (error) {
