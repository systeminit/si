...
Deno.test(
  "executeChanges - should throw error when API config not available",
  async () => {
...
Deno.test(
  "executeChanges - handles empty changes array (requires mocking)",
  async () => {
...
Deno.test(
  "executeChanges - continues execution on error (requires mocking)",
  async () => {
...
Deno.test(
  "resolveSubscriptionPlaceholders - resolves workingSet IDs (tested indirectly)",
  async () => {
...
Deno.test(
  "getOrCreateChangeSet - finds existing change set (requires mocking)",
  async () => {
...
Deno.test(
  "getOrCreateChangeSet - creates new change set (requires mocking)",
  async () => {
    // This test would require mocking the ChangeSetsApi
    // Skipped for now - structure here for when we add mocking support
  },
...
Deno.test(
  "executeUpdate - sends attribute diff payload (requires mocking)",
  async () => {
    // This test would require mocking the ComponentsApi
    // Skipped for now - structure here for when we add mocking support
  },
...
Deno.test(
  "executeDelete - calls deleteComponent (requires mocking)",
  async () => {
    // This test would require mocking the ComponentsApi
    // Skipped for now - structure here for when we add mocking support
  },
...
Deno.test(
  "executeChanges integration - executes all change types in order (requires mocking)",
  async () => {
    // This would require:
...
Deno.test(
  "executeChanges integration - handles mixed subscriptions (requires mocking)",
  async () => {
