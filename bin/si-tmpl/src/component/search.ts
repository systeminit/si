  ...
  if (outputFormat === "json") {
    ...
    for (let i = 0; i < output.components.length; i++) {
      ...
      if (componentData.fullComponent && options.fullComponent) {
        const fullComponent = componentData.fullComponent as {
          id: string;
          schemaId: string;
          name: string;
          resourceId?: string;
        ...
        const componentActions = allActions
          ? filterActionsByComponent(allActions, fullComponent.id)
          : [];
        ...
        // Display using shared display function
