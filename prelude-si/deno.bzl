...
deno_cache = rule(
    ...
    attrs = {
        ...
        "workspace": attrs.dep(
            doc = "The assembled deno_workspace target.",
        ),
...
deno_compile = rule(
    ...
    attrs = {
        ...
        "srcs": attrs.list(attrs.source(), default = [], doc = "All source files that are part of the compilation"),
        ...
        "permissions": attrs.list(attrs.string(), default = [], doc = "List of Deno permissions to grant (e.g., read, write, net)"),
        "unstable_flags": attrs.list(attrs.string(), default = [], doc = "List of unstable flags to enable"),
        "_python_toolchain": attrs.toolchain_dep(default = "toolchains//:python", providers = [PythonToolchainInfo]),
        "_deno_toolchain": attrs.toolchain_dep(default = "toolchains//:deno", providers = [DenoToolchainInfo]),
    },
)
...
def _deno_workspace_impl(ctx: AnalysisContext) -> list[Provider]:
    ...
    workspace_out = ctx.actions.declare_output(ctx.attrs.workspace_out_name, dir = True)
    ...
    cmd.add("--root-config", ctx.attrs.root_config[DefaultInfo].default_outputs[0])
