...
load(":mise_buildscript_wrapper.bzl", "mise_buildscript_runner")
...
def _make_rustc_shim(ctx: AnalysisContext, cwd: Artifact) -> cmd_args:
    ...
    if explicit_sysroot_deps:
        dep_ctx = DepCollectionContext(advanced_unstable_linking = False, include_doc_deps = False, is_proc_macro = False, explicit_sysroot_deps = explicit_sysroot_deps, panic_runtime = PanicRuntime("unwind"))
        deps = gather_explicit_sysroot_deps(dep_ctx)
        deps = resolve_rust_deps_inner(ctx, deps)
        dep_args, _ = dependency_args(ctx = ctx, compile_ctx = None, toolchain_info = toolchain_info, deps = deps, subdir = "any", dep_link_strategy = DEFAULT_STATIC_LINK_STRATEGY, dep_metadata_kind = MetadataKind("full"), is_rustdoc_test = False)
    ...
    shim = cmd_script(ctx = ctx, name = "__rustc_shim", cmd = cmd_args(toolchain_info.compiler, sysroot_args, relative_to = cwd), language = ctx.attrs._exec_os_type[OsLookup].script)
...
_si_cargo_buildscript_rule = rule(
    ...
    attrs = {
        ...
        "runner": attrs.exec_dep(providers = [RunInfo], default = "prelude//rust/tools:buildscript_run"),
...
def buildscript_run(
        ...
        manifest_dir = None,
        **kwargs):
    # This logic for finding the manifest directory is important and restored from the prelude
