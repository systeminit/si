    ...
    in
      with pkgs; rec {
        ...
        devShells.default = mkShell {
          # TEMPORARY: Setup Buck2 config for remote builds in CI only
          # TODO: Remove this shellHook when CI migration is complete
          shellHook = ''
            if [ -n "$BUILDKITE_BUILD_ID" ]; then
              echo "[buildkite]" > .buckconfig.local
              echo "enabled = true" >> .buckconfig.local
              echo "# Generated by Nix flake for CI build ($BUILDKITE_BUILD_ID)" >> .buckconfig.local
              echo "Buck2 remote builds ENABLED for CI"
            fi
          '';
