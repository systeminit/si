      ...
      cy.get("@profile").then((profile: Record<string, any>) => {
        cy.get("@changeSet").then((changeSet: Record<string, any>) => {
          cy.graphqlMutation({
            ...
            variables: {
              changeSetId: changeSet["data"]["changeSetCreate"]["item"]["id"],
              ...generateEntity(profile["workspaceDefault"]["id"], 0),
            },
      ...
      cy.get("@profile").then((profile: Record<string, any>) => {
        cy.get("@changeSet").then((changeSet: Record<string, any>) => {
          ...
          cy.graphqlQuery({
            ...
            const responseData =
              results["data"]["kubernetesDeploymentEntityList"];
            expect(responseData).to.have.property("items");
        ...
        cy.get("@changeSet").then((changeSet: Record<string, any>) => {
          for (let count = 0; count < 50; count++) {
            cy.graphqlMutation({
              ...
              variables: {
                changeSetId: changeSet["data"]["changeSetCreate"]["item"]["id"],
                ...generateEntity(profile["workspaceDefault"]["id"], count),
              },
            });
          }
          cy.graphqlQuery({
