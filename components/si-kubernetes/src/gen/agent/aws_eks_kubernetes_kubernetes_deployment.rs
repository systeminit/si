...
pub struct AwsEksKubernetesKubernetesDeploymentDispatcherBuilder<
...
impl<T> si_agent::DispatchBuilder for AwsEksKubernetesKubernetesDeploymentDispatcherBuilder<T>
where
    T: AwsEksKubernetesKubernetesDeploymentDispatchFunctions + Sync + Send + 'static,
{
    type Dispatchable = AwsEksKubernetesKubernetesDeploymentDispatcher<T>;
...
impl<T> AwsEksKubernetesKubernetesDeploymentDispatcher<T>
where
    T: AwsEksKubernetesKubernetesDeploymentDispatchFunctions,
{
...
impl<T> si_agent::Dispatch for AwsEksKubernetesKubernetesDeploymentDispatcher<T>
where
    T: AwsEksKubernetesKubernetesDeploymentDispatchFunctions + Sync + Send,
{
    async fn dispatch(
        &self,
        transport: &si_agent::Transport,
        ...
        let (_header, _qos, response_header, mut entity_event) = {
            let msg: si_agent::Message<crate::protobuf::KubernetesDeploymentEntityEvent> =
                message.try_into()?;
            msg.into_parts()
        };
...
impl<T> si_agent::Dispatchable for AwsEksKubernetesKubernetesDeploymentDispatcher<T> where
    T: AwsEksKubernetesKubernetesDeploymentDispatchFunctions + Sync + Send
{
}
