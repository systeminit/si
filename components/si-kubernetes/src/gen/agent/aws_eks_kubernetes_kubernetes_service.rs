...
pub struct AwsEksKubernetesKubernetesServiceDispatcherBuilder<
...
impl<T> si_agent::DispatchBuilder for AwsEksKubernetesKubernetesServiceDispatcherBuilder<T>
where
    T: AwsEksKubernetesKubernetesServiceDispatchFunctions + Sync + Send + 'static,
{
    type Dispatchable = AwsEksKubernetesKubernetesServiceDispatcher<T>;
...
impl<T> AwsEksKubernetesKubernetesServiceDispatcher<T>
where
    T: AwsEksKubernetesKubernetesServiceDispatchFunctions,
{
...
impl<T> si_agent::Dispatch for AwsEksKubernetesKubernetesServiceDispatcher<T>
where
    T: AwsEksKubernetesKubernetesServiceDispatchFunctions + Sync + Send,
{
    async fn dispatch(
        &self,
        transport: &si_agent::Transport,
        ...
        let (_header, _qos, response_header, mut entity_event) = {
            let msg: si_agent::Message<crate::protobuf::KubernetesServiceEntityEvent> =
                message.try_into()?;
            msg.into_parts()
        };
...
impl<T> si_agent::Dispatchable for AwsEksKubernetesKubernetesServiceDispatcher<T> where
    T: AwsEksKubernetesKubernetesServiceDispatchFunctions + Sync + Send
{
}
