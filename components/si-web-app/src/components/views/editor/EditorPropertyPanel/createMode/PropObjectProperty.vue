        ...
        <div v-if="propObjectProperty.kind() == 'link'">
          <div v-if="propObjectProperty.lookupMyself().kind() == 'object'">
            <div class="flex flex-row">
              ...
              <div>
                <div
                  v-for="(object, index) in objectModel"
                  :key="index"
                  class="flex pt-1"
                >
                  <PropObject
                ...
                <div class="flex text-gray-500 pt-1">
                  <button
                    class="text-center w-4 focus:outline-none"
                    type="button"
                    @click="onClickB($event, propObjectProperty)"
                  >
...
export default Vue.extend({
  ...
  watch: {
    objectModel(newVal, _oldVal) {
      console.log(
        "PropObjectProperty.watch.objectModel() with ::",
        this.propObjectProperty.name,
        newVal,
      );
      ...
      switch (propObjectProperty.kind()) {
        case "link":
          ...
          if (propObjectProperty.lookupMyself().kind() == "object") {
            ...
            varsObjectForProperty = variablesObjectForProperty(
              propObjectProperty.lookupMyself(),
              true,
            );
