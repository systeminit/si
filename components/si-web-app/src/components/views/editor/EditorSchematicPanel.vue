<template>
  <div id="schematic-panel" class="h-full w-full grid-background">
    <div ref="canvasParent" class="w-full h-full overflow-hidden" @wheel="mouseWheel" @mousedown="mouseDown" @mousemove="mouseMove" @mouseup="mouseUp">
      <div ref="canvas" id="canvas" class="flex-auto relative w-full h-full canvas block">

        <div class="node draggable absolute">Node</div>

        <svg ref="grid" :width="gridWidth" :height="gridHeight" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="grid" width="20" height="20" patternUnits="userSpaceOnUse" class="grid-background">
              <rect fill="#111718" width="20" height="20"/>
              <path d="M 20 0 L 0 0 0 20" fill="none" stroke="#76A1A9" stroke-width="1"/>
            </pattern>
          </defs>
  ...
  methods: {
    ...
    mouseDown(event) {
      ...
      if(event.altKey) {
        ...
        this.log(`mousePositionInScreenSpace - x:${mousePositionInScreenSpace.x} y:${mousePositionInScreenSpace.y}`)
    ...
    canvasConstraint() {
      ...
      console.log("::" + this.grid.element.getBoundingClientRect().left, this.grid.element.getBoundingClientRect().top)
      console.log("::" + this.canvas.element.offsetLeft, this.canvas.element.offsetTop)
    ...
    panCanvas() {
      ...
      console.log(`canvas offset - x:${this.canvas.offset.x} y:${this.canvas.offset.y}`)
      this.log(`desired adjusted translation - x:${translation.x} y:${translation.y}`)
      this.log(`translation limits - x:0 to ${(1 - this.editor.element.offsetWidth)} y:0 to ${(1 - this.editor.element.offsetHeight)}`)

      // Restrict translations to the view
      translation.x = Math.min(0, translation.x)
      translation.x = Math.max((1 - this.editor.element.offsetWidth), translation.x)

      translation.y = Math.min(0, translation.y)
      translation.y = Math.max((1 - this.editor.element.offsetHeight), translation.y)
    ...
    zoomCanvas(event) {
      ...
      let mousePositionInCanvasSpace = {
        x: (mousePositionInScreenSpace.x - this.zoom.translation.x) * zoomDeltaPercentage,
        y: (mousePositionInScreenSpace.y - this.zoom.translation.y) * zoomDeltaPercentage
      }
