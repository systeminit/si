<template>
  <div id="system-details" class="application-list-bg-color flex flex-col flex-no-wrap">
    
    <div class="flex flex-row mt-4 h-10 application-list-menu-bar">

        <button class="bg-teal-700 ml-4 px-2 h-8 mt-1 text-white hover:bg-teal-600" @click="showModal()" type="button">
          new application
        </button>

    </div>

    <modal name="hello-world" width="500" height="100" draggable styles="background-color:#313436">
      ...
      <div class="flex flex-col">
        <div class="flex flex-row mx-2 my-2">
          ...
          <button class="bg-teal-700 ml-4 mt-4 w-16 text-white hover:bg-teal-600" @click="newApp(applicationName)" type="button">
            create
          </button>
      </div>
    ...
    <div v-if="applications" class="mx-4">
      ...
      <table class="table-auto text-white">
        ...
        <tbody v-for="app in applications" :key="app.id">
          <tr class="border border-teal-800 px-4 py-2" @click="clickApp(app.id)">
            <td >{{app.name}}</td>
          </tr>
        </tbody>
      </table>
