<template>
  <div :loading="$apollo.loading">
    ...
    <form class="bg-primary shadow-md rounded px-12 pt-6 pb-8 mb-4">
      <p class="text-center text-xl justify-center text-white mb-4">
        Sign In to The System Initiative
      </p>

      <div class="mb-4">
        <div
          v-for="field of objMethod.properties.attrs"
        ...
        >
          <div v-if="field.kind() == 'text'">
            ...
            <input
              class="block login-input-bg-color border border-gray-700 text-red-100 text-sm font-bold mb-2"
          ...
          <div v-else-if="field.kind() == 'password'">
            <label
              class="block border-gray-300 text-gray-200 text-sm font-bold mb-2"
              for="password"
            >
              {{ field.label }}
            </label>
            ...
            <input
              class="block border login-input-bg-color border-gray-700 text-gray-700 text-sm font-bold mb-2"
              v-model="objVariables[field.name]"
              :label="field.label"
              :name="field.name"
              type="password"
        ...
        <button
          class="logint-button-bg-color-dark hover:login-button-bg-color-light text-gray-200 font-bold py-1 px-4 focus:outline-none focus:shadow-outline"
          @click="checkLogin()"
          type="button"
        >
          SignIn
    ...
    async checkLogin() {
      try {
        ...
        billingAccountList.addAccount({
          ...
          name: this.objVariables.billingAccountName,
        });
