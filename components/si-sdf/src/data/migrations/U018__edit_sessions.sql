...
BEGIN
    ...
    INSERT INTO edit_sessions (id, si_id, name, billing_account_id, organization_id, workspace_id, change_set_id, epoch,
                               update_count, tenant_ids, obj, reverted, created_at, updated_at)
    VALUES (this_id, si_id, this_name, this_billing_account_id, this_organization_id, this_workspace_id,
            this_change_set_id, this_epoch, this_update_count, tenant_ids, object, false, created_at, updated_at);
...
BEGIN
    ...
    SET name         = input_edit_session ->> 'name',
        ...
        epoch        = (input_edit_session -> 'siStorable' -> 'updateClock' ->> 'epoch')::bigint,
        update_count = (input_edit_session -> 'siStorable' -> 'updateClock' ->> 'updateCount')::bigint,
...
$$
    ...
    BEGIN
        ...
        UPDATE edit_sessions SET reverted = true,
                                 obj = this_edit_session_obj,
                                 updated_at = now()
        WHERE id = this_edit_session_id;

        UPDATE ops SET obj = jsonb_set(obj, '{siOp, skip}', 'true'::jsonb),
                       updated_at = now()
