...
CREATE OR REPLACE FUNCTION node_create_v1(this_kind text,
...
BEGIN
    ...
    INSERT INTO nodes (id, si_id, kind, object_type, object_si_id, billing_account_id, organization_id, workspace_id,
                       epoch,
                       update_count, tenant_ids, obj, created_at, updated_at)
    VALUES (this_id, si_id, this_kind, this_object_type, 'sovietRussiaPoopCanoeOwnsYou', this_billing_account_id,
            this_organization_id, this_workspace_id, this_epoch, this_update_count, tenant_ids, object, created_at,
            updated_at);
END;
...
BEGIN
    ...
    UPDATE nodes
    SET epoch        = (input_node -> 'siStorable' -> 'updateClock' ->> 'epoch')::bigint,
        update_count = (input_node -> 'siStorable' -> 'updateClock' ->> 'updateCount')::bigint,
        obj          = input_node,
        updated_at   = NOW()
    WHERE id = this_id
    RETURNING obj INTO object;
