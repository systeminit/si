...
BEGIN
    ...
    INSERT INTO event_logs (id, si_id, billing_account_id, organization_id, workspace_id, event_id, epoch, update_count,
                            tenant_ids, obj, created_at, updated_at)
    VALUES (this_id, si_id, this_billing_account_id, this_organization_id, this_workspace_id,
            si_id_to_primary_key_v1(this_event_si_id), this_epoch, this_update_count, tenant_ids, object, created_at,
            updated_at);
END;
...
BEGIN
    ...
    UPDATE event_logs
    SET epoch = (input_event_log -> 'siStorable' -> 'updateClock' ->> 'epoch')::bigint,
        update_count = (input_event_log -> 'siStorable' -> 'updateClock' ->> 'updateCount')::bigint,
        obj        = input_event_log,
        updated_at = NOW()
    WHERE id = this_id
