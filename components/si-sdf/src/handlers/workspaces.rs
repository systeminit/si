...
use crate::models::{ListRequest, PageToken, Query, Workspace};
...
pub async fn get(
    ...
    validate_tenancy(&txn, "workspaces", &workspace_id, &claim.billing_account_id).await?;
    authorize(&txn, &claim.user_id, "workspaces", "get").await?;

    let object = Workspace::get(&txn, &workspace_id)
        .await
...
pub async fn list(
    ...
    let claim = authenticate(&txn, &token).await?;
    authorize(&txn, &claim.user_id, "workspaces", "list").await?;
