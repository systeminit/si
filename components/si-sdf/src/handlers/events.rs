...
use crate::models::{Event, ListRequest, PageToken, Query};
...
pub async fn get(
    ...
    validate_tenancy(&txn, "events", &event_id, &claim.billing_account_id).await?;
    authorize(&txn, &claim.user_id, "event", "get").await?;

    let object = Event::get(&txn, &event_id)
        .await
...
pub async fn list(
    ...
    let claim = authenticate(&txn, &token).await?;
    authorize(&txn, &claim.user_id, "event", "list").await?;
