use crate::protobuf::AmiComponentConstraints;
use si_cea::component::prelude::*;

type Constraints = AmiComponentConstraints;

pub use crate::protobuf::AmiComponent;

impl AmiComponent {
    pub async fn pick(db: &Db, constraints: Option<Constraints>) -> CeaResult<(Constraints, Self)> {
        let span = tracing_pick_span!("ami", &constraints);
        async {
