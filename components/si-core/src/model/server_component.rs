use crate::protobuf::ServerComponentConstraints;
use si_cea::component::prelude::*;

type Constraints = ServerComponentConstraints;

pub use crate::protobuf::ServerComponent;

impl ServerComponent {
    pub async fn pick(db: &Db, constraints: Option<Constraints>) -> CeaResult<(Constraints, Self)> {
        let span = tracing_pick_span!("server", &constraints);
        async {
