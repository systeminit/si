use crate::protobuf::ServiceComponentConstraints;
use si_cea::component::prelude::*;

type Constraints = ServiceComponentConstraints;

pub use crate::protobuf::ServiceComponent;

impl ServiceComponent {
    pub async fn pick(db: &Db, constraints: Option<Constraints>) -> CeaResult<(Constraints, Self)> {
        let span = tracing_pick_span!("service", &constraints);
        async {
