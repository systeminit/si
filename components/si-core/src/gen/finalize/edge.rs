...
struct EdgeFinalizerBuilder {
    finalize_key: Option<si_cea::FinalizeKey>,
}

impl si_cea::FinalizeBuilder for EdgeFinalizerBuilder {
    type Finalizeable = EdgeFinalizer;
...
impl si_cea::Finalize for EdgeFinalizer {
    async fn finalize(
        &self,
        db: &si_data::Db,
        ...
        let (_header, _qos, _response_header, object) = {
            let msg: si_agent::Message<crate::protobuf::Edge> = message.try_into()?;
            msg.into_parts()
        };
...
impl si_cea::Finalizeable for EdgeFinalizer {}

pub fn finalizer() -> si_cea::CeaResult<EdgeFinalizer> {
    let mut builder = EdgeFinalizerBuilder::new();
