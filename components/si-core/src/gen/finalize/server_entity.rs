...
struct ServerEntityFinalizerBuilder {
    finalize_key: Option<si_cea::FinalizeKey>,
}

impl si_cea::FinalizeBuilder for ServerEntityFinalizerBuilder {
    type Finalizeable = ServerEntityFinalizer;
...
pub struct ServerEntityFinalizer {
    finalize_key: si_cea::FinalizeKey,
}

impl ServerEntityFinalizer {
...
impl si_cea::Finalize for ServerEntityFinalizer {
    async fn finalize(
        &self,
        db: &si_data::Db,
        ...
        let (_header, _qos, _response_header, object) = {
            let msg: si_agent::Message<crate::protobuf::ServerEntity> = message.try_into()?;
            msg.into_parts()
        };
...
impl si_cea::Finalizeable for ServerEntityFinalizer {}

pub fn finalizer() -> si_cea::CeaResult<ServerEntityFinalizer> {
    let mut builder = ServerEntityFinalizerBuilder::new();
