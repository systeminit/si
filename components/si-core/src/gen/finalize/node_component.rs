...
struct NodeComponentFinalizerBuilder {
    finalize_key: Option<si_cea::FinalizeKey>,
}

impl si_cea::FinalizeBuilder for NodeComponentFinalizerBuilder {
    type Finalizeable = NodeComponentFinalizer;
...
pub struct NodeComponentFinalizer {
    finalize_key: si_cea::FinalizeKey,
}

impl NodeComponentFinalizer {
...
impl si_cea::Finalize for NodeComponentFinalizer {
    async fn finalize(
        &self,
        db: &si_data::Db,
        ...
        let (_header, _qos, _response_topic, object) = {
            let msg: si_agent::Message<crate::protobuf::NodeComponent> = message.try_into()?;
            msg.into_parts()
        };
...
impl si_cea::Finalizeable for NodeComponentFinalizer {}

pub fn finalizer() -> si_cea::CeaResult<NodeComponentFinalizer> {
    let mut builder = NodeComponentFinalizerBuilder::new();
