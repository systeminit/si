...
struct NodeFinalizerBuilder {
    finalize_key: Option<si_cea::FinalizeKey>,
}

impl si_cea::FinalizeBuilder for NodeFinalizerBuilder {
    type Finalizeable = NodeFinalizer;
...
impl si_cea::Finalize for NodeFinalizer {
    async fn finalize(
        &self,
        db: &si_data::Db,
        ...
        let (_header, _qos, _response_header, object) = {
            let msg: si_agent::Message<crate::protobuf::Node> = message.try_into()?;
            msg.into_parts()
        };
...
impl si_cea::Finalizeable for NodeFinalizer {}

pub fn finalizer() -> si_cea::CeaResult<NodeFinalizer> {
    let mut builder = NodeFinalizerBuilder::new();
