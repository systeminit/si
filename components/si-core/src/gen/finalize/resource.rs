...
struct ResourceFinalizerBuilder {
    finalize_key: Option<si_cea::FinalizeKey>,
}

impl si_cea::FinalizeBuilder for ResourceFinalizerBuilder {
    type Finalizeable = ResourceFinalizer;
...
impl si_cea::Finalize for ResourceFinalizer {
    async fn finalize(
        &self,
        db: &si_data::Db,
        ...
        let (_header, _qos, _response_header, object) = {
            let msg: si_agent::Message<crate::protobuf::Resource> = message.try_into()?;
            msg.into_parts()
        };
...
impl si_cea::Finalizeable for ResourceFinalizer {}

pub fn finalizer() -> si_cea::CeaResult<ResourceFinalizer> {
    let mut builder = ResourceFinalizerBuilder::new();
