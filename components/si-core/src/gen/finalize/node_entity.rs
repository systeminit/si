...
struct NodeEntityFinalizerBuilder {
    finalize_key: Option<si_cea::FinalizeKey>,
}

impl si_cea::FinalizeBuilder for NodeEntityFinalizerBuilder {
    type Finalizeable = NodeEntityFinalizer;
...
pub struct NodeEntityFinalizer {
    finalize_key: si_cea::FinalizeKey,
}

impl NodeEntityFinalizer {
...
impl si_cea::Finalize for NodeEntityFinalizer {
    async fn finalize(
        &self,
        db: &si_data::Db,
        ...
        let (_header, _qos, _response_topic, object) = {
            let msg: si_agent::Message<crate::protobuf::NodeEntity> = message.try_into()?;
            msg.into_parts()
        };
...
impl si_cea::Finalizeable for NodeEntityFinalizer {}

pub fn finalizer() -> si_cea::CeaResult<NodeEntityFinalizer> {
    let mut builder = NodeEntityFinalizerBuilder::new();
