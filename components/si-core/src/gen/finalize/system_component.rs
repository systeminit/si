...
struct SystemComponentFinalizerBuilder {
    finalize_key: Option<si_cea::FinalizeKey>,
}

impl si_cea::FinalizeBuilder for SystemComponentFinalizerBuilder {
    type Finalizeable = SystemComponentFinalizer;
...
pub struct SystemComponentFinalizer {
    finalize_key: si_cea::FinalizeKey,
}

impl SystemComponentFinalizer {
...
impl si_cea::Finalize for SystemComponentFinalizer {
    async fn finalize(
        &self,
        db: &si_data::Db,
        ...
        let (_header, _qos, _response_topic, object) = {
            let msg: si_agent::Message<crate::protobuf::SystemComponent> = message.try_into()?;
            msg.into_parts()
        };
...
impl si_cea::Finalizeable for SystemComponentFinalizer {}

pub fn finalizer() -> si_cea::CeaResult<SystemComponentFinalizer> {
    let mut builder = SystemComponentFinalizerBuilder::new();
