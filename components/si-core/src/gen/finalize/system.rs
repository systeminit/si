...
struct SystemFinalizerBuilder {
    finalize_key: Option<si_cea::FinalizeKey>,
}

impl si_cea::FinalizeBuilder for SystemFinalizerBuilder {
    type Finalizeable = SystemFinalizer;
...
pub struct SystemFinalizer {
    finalize_key: si_cea::FinalizeKey,
}

impl SystemFinalizer {
...
impl si_cea::Finalize for SystemFinalizer {
    async fn finalize(
        &self,
        db: &si_data::Db,
        ...
        let (_header, _qos, _response_header, object) = {
            let msg: si_agent::Message<crate::protobuf::System> = message.try_into()?;
            msg.into_parts()
        };
...
impl si_cea::Finalizeable for SystemFinalizer {}

pub fn finalizer() -> si_cea::CeaResult<SystemFinalizer> {
    let mut builder = SystemFinalizerBuilder::new();
