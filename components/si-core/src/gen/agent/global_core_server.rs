...
pub struct GlobalCoreServerDispatcherBuilder<T: GlobalCoreServerDispatchFunctions> {
    dispatch_key: Option<si_agent::DispatchKey>,
    _phantom: std::marker::PhantomData<T>,
}

impl<T> si_agent::DispatchBuilder for GlobalCoreServerDispatcherBuilder<T>
where
    T: GlobalCoreServerDispatchFunctions + Sync + Send + 'static,
{
    type Dispatchable = GlobalCoreServerDispatcher<T>;
...
pub struct GlobalCoreServerDispatcher<T: GlobalCoreServerDispatchFunctions> {
    dispatch_key: si_agent::DispatchKey,
    _phantom: std::marker::PhantomData<T>,
}

impl<T> GlobalCoreServerDispatcher<T>
where
    T: GlobalCoreServerDispatchFunctions,
{
    ...
    async fn dispatch_event(
        ...
        entity_event: &mut crate::protobuf::ServerEntityEvent,
...
impl<T> si_agent::Dispatch for GlobalCoreServerDispatcher<T>
where
    T: GlobalCoreServerDispatchFunctions + Sync + Send,
{
    async fn dispatch(
        &self,
        transport: &si_agent::Transport,
        ...
        let (_header, _qos, response_header, mut entity_event) = {
            let msg: si_agent::Message<crate::protobuf::ServerEntityEvent> = message.try_into()?;
            msg.into_parts()
        };
...
impl<T> si_agent::Dispatchable for GlobalCoreServerDispatcher<T> where
    T: GlobalCoreServerDispatchFunctions + Sync + Send
{
}

#[async_trait::async_trait]
pub trait GlobalCoreServerDispatchFunctions {
    async fn create(
        transport: &si_agent::Transport,
        stream_header: si_agent::Header,
        entity_event: &mut crate::protobuf::ServerEntityEvent,
