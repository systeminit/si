...
pub struct GlobalCoreEdgeDispatcherBuilder<T: GlobalCoreEdgeDispatchFunctions> {
    dispatch_key: Option<si_agent::DispatchKey>,
    _phantom: std::marker::PhantomData<T>,
}

impl<T> si_agent::DispatchBuilder for GlobalCoreEdgeDispatcherBuilder<T>
where
    T: GlobalCoreEdgeDispatchFunctions + Sync + Send + 'static,
{
    type Dispatchable = GlobalCoreEdgeDispatcher<T>;
...
impl<T> GlobalCoreEdgeDispatcher<T>
where
    T: GlobalCoreEdgeDispatchFunctions,
{
    ...
    async fn dispatch_event(
        ...
        entity_event: &mut crate::protobuf::EdgeEntityEvent,
...
impl<T> si_agent::Dispatch for GlobalCoreEdgeDispatcher<T>
where
    T: GlobalCoreEdgeDispatchFunctions + Sync + Send,
{
    async fn dispatch(
        &self,
        transport: &si_agent::Transport,
...
impl<T> si_agent::Dispatchable for GlobalCoreEdgeDispatcher<T> where
    T: GlobalCoreEdgeDispatchFunctions + Sync + Send
{
}
