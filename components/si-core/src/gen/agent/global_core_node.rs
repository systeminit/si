...
pub struct GlobalCoreNodeDispatcherBuilder<T: GlobalCoreNodeDispatchFunctions> {
    dispatch_key: Option<si_agent::DispatchKey>,
    _phantom: std::marker::PhantomData<T>,
}

impl<T> si_agent::DispatchBuilder for GlobalCoreNodeDispatcherBuilder<T>
where
    T: GlobalCoreNodeDispatchFunctions + Sync + Send + 'static,
{
    type Dispatchable = GlobalCoreNodeDispatcher<T>;
...
pub struct GlobalCoreNodeDispatcher<T: GlobalCoreNodeDispatchFunctions> {
    dispatch_key: si_agent::DispatchKey,
    _phantom: std::marker::PhantomData<T>,
}

impl<T> GlobalCoreNodeDispatcher<T>
where
    T: GlobalCoreNodeDispatchFunctions,
{
    ...
    async fn dispatch_event(
        ...
        entity_event: &mut crate::protobuf::NodeEntityEvent,
...
impl<T> si_agent::Dispatch for GlobalCoreNodeDispatcher<T>
where
    T: GlobalCoreNodeDispatchFunctions + Sync + Send,
{
    async fn dispatch(
        &self,
        transport: &si_agent::Transport,
        ...
        let (_header, _qos, response_topic, mut entity_event) = {
            let msg: si_agent::Message<crate::protobuf::NodeEntityEvent> = message.try_into()?;
            msg.into_parts()
        };
...
impl<T> si_agent::Dispatchable for GlobalCoreNodeDispatcher<T> where
    T: GlobalCoreNodeDispatchFunctions + Sync + Send
{
}

#[async_trait::async_trait]
pub trait GlobalCoreNodeDispatchFunctions {
    async fn create(
        transport: &si_agent::Transport,
        stream_header: si_agent::Header,
        entity_event: &mut crate::protobuf::NodeEntityEvent,
