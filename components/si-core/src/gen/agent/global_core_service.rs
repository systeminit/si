...
pub struct GlobalCoreServiceDispatcherBuilder<T: GlobalCoreServiceDispatchFunctions> {
    dispatch_key: Option<si_agent::DispatchKey>,
    _phantom: std::marker::PhantomData<T>,
}

impl<T> si_agent::DispatchBuilder for GlobalCoreServiceDispatcherBuilder<T>
where
    T: GlobalCoreServiceDispatchFunctions + Sync + Send + 'static,
{
    type Dispatchable = GlobalCoreServiceDispatcher<T>;
...
impl<T> GlobalCoreServiceDispatcher<T>
where
    T: GlobalCoreServiceDispatchFunctions,
{
    ...
    async fn dispatch_event(
        ...
        entity_event: &mut crate::protobuf::ServiceEntityEvent,
            ...
            .map_err(si_agent::Error::execute)?
        {
            ...
            "deploy" => T::deploy(transport, stream_header, entity_event).await,
...
impl<T> si_agent::Dispatch for GlobalCoreServiceDispatcher<T>
where
    T: GlobalCoreServiceDispatchFunctions + Sync + Send,
{
    async fn dispatch(
        &self,
        transport: &si_agent::Transport,
...
impl<T> si_agent::Dispatchable for GlobalCoreServiceDispatcher<T> where
    T: GlobalCoreServiceDispatchFunctions + Sync + Send
{
}
