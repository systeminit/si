...
pub struct AwsEksKubernetesUbuntuDispatcherBuilder<T: AwsEksKubernetesUbuntuDispatchFunctions> {
    dispatch_key: Option<si_agent::DispatchKey>,
    _phantom: std::marker::PhantomData<T>,
}

impl<T> si_agent::DispatchBuilder for AwsEksKubernetesUbuntuDispatcherBuilder<T>
where
    T: AwsEksKubernetesUbuntuDispatchFunctions + Sync + Send + 'static,
{
    type Dispatchable = AwsEksKubernetesUbuntuDispatcher<T>;
...
pub struct AwsEksKubernetesUbuntuDispatcher<T: AwsEksKubernetesUbuntuDispatchFunctions> {
    dispatch_key: si_agent::DispatchKey,
    _phantom: std::marker::PhantomData<T>,
}

impl<T> AwsEksKubernetesUbuntuDispatcher<T>
where
    T: AwsEksKubernetesUbuntuDispatchFunctions,
{
    ...
    async fn dispatch_event(
        ...
        entity_event: &mut crate::protobuf::UbuntuEntityEvent,
            ...
            .map_err(si_agent::Error::execute)?
        {
            ...
            "halt" => T::halt(transport, stream_header, entity_event).await,
...
impl<T> si_agent::Dispatch for AwsEksKubernetesUbuntuDispatcher<T>
where
    T: AwsEksKubernetesUbuntuDispatchFunctions + Sync + Send,
{
    async fn dispatch(
        &self,
        transport: &si_agent::Transport,
        ...
        let (_header, _qos, response_header, mut entity_event) = {
            let msg: si_agent::Message<crate::protobuf::UbuntuEntityEvent> = message.try_into()?;
            msg.into_parts()
        };
...
impl<T> si_agent::Dispatchable for AwsEksKubernetesUbuntuDispatcher<T> where
    T: AwsEksKubernetesUbuntuDispatchFunctions + Sync + Send
{
}

#[async_trait::async_trait]
pub trait AwsEksKubernetesUbuntuDispatchFunctions {
    async fn create(
        transport: &si_agent::Transport,
        stream_header: si_agent::Header,
        entity_event: &mut crate::protobuf::UbuntuEntityEvent,
