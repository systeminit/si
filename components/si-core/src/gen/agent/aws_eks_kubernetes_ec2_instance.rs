...
pub struct AwsEksKubernetesEc2InstanceDispatcherBuilder<
    T: AwsEksKubernetesEc2InstanceDispatchFunctions,
> {
    dispatch_key: Option<si_agent::DispatchKey>,
    _phantom: std::marker::PhantomData<T>,
}

impl<T> si_agent::DispatchBuilder for AwsEksKubernetesEc2InstanceDispatcherBuilder<T>
where
    T: AwsEksKubernetesEc2InstanceDispatchFunctions + Sync + Send + 'static,
{
    type Dispatchable = AwsEksKubernetesEc2InstanceDispatcher<T>;
    ...
    fn object_type(&self) -> &'static str {
        "ec2_instance_entity_event"
    }
}

pub struct AwsEksKubernetesEc2InstanceDispatcher<T: AwsEksKubernetesEc2InstanceDispatchFunctions> {
    dispatch_key: si_agent::DispatchKey,
    _phantom: std::marker::PhantomData<T>,
}

impl<T> AwsEksKubernetesEc2InstanceDispatcher<T>
where
    T: AwsEksKubernetesEc2InstanceDispatchFunctions,
{
    ...
    async fn dispatch_event(
        ...
        entity_event: &mut crate::protobuf::Ec2InstanceEntityEvent,
            ...
            .map_err(si_agent::Error::execute)?
        {
            ...
            "launch" => T::launch(transport, stream_header, entity_event).await,
            "reboot" => T::reboot(transport, stream_header, entity_event).await,
            ...
            "terminate" => T::terminate(transport, stream_header, entity_event).await,
...
impl<T> si_agent::Dispatch for AwsEksKubernetesEc2InstanceDispatcher<T>
where
    T: AwsEksKubernetesEc2InstanceDispatchFunctions + Sync + Send,
{
    async fn dispatch(
        &self,
        transport: &si_agent::Transport,
        ...
        let (_header, _qos, response_header, mut entity_event) = {
            let msg: si_agent::Message<crate::protobuf::Ec2InstanceEntityEvent> =
                message.try_into()?;
            msg.into_parts()
        };
...
impl<T> si_agent::Dispatchable for AwsEksKubernetesEc2InstanceDispatcher<T> where
    T: AwsEksKubernetesEc2InstanceDispatchFunctions + Sync + Send
{
}

#[async_trait::async_trait]
pub trait AwsEksKubernetesEc2InstanceDispatchFunctions {
    async fn create(
        transport: &si_agent::Transport,
        stream_header: si_agent::Header,
        entity_event: &mut crate::protobuf::Ec2InstanceEntityEvent,
