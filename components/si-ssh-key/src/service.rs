...
use crate::protobuf::{
    ...
    ssh_key_server, ConstraintValueReply,
};
...
impl ssh_key_server::SshKey for Service {
    async fn pick_component(
        ...
        if req.name != "" {
            ...
            if name_check_result.len() == 1 {
                ...
                return Ok(Response::new(protobuf::PickComponentReply {
                    ...
                    chosen: name_check_result.items.pop(),
                    ..Default::default()
                }));
            } else {
        ...
        let mut implicit_constraints: Vec<ConstraintValueReply> = Vec::new();
        ...
        if req.key_type == 0 {
            ...
            implicit_constraints.push(ConstraintValueReply {
