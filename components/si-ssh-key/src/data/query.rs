...
impl Query {
    pub fn as_n1ql(&self) -> Result<String> {
        ...
        for query_e_option in self.items.iter() {
            if item_count != 0 {
                let qbl = match QueryBooleanLogic::from_i32(self.boolean_term) {
                    Some(l) => l,
                    None => return Err(Error::InvalidBooleanLogic),
                };
            ...
            match &query_e_option.qe {
                Some(query_expression_option::Qe::Expression(exp)) => {
                    let comparison = match QueryComparison::from_i32(exp.comparison) {
                        Some(c) => c,
                        None => return Err(Error::InvalidQueryComparison),
                    };
                    let value = match QueryFieldType::from_i32(exp.field_type) {
                        ...
                        None => return Err(Error::InvalidFieldType),
                    };
                    let expression = format!("si.[{}] {} {}", json![exp.field], comparison, value);
                    where_string.push_str(&expression);
                }
                Some(query_expression_option::Qe::Query(q)) => {
                    let query_group = q.as_n1ql()?;
...
mod query_test {
    ...
    fn single_term() {
        ...
        let query_string = query.as_n1ql().expect("Failed to create n1ql query");
